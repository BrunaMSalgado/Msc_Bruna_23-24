...
Considering $\phi_2=\phi_2'$, $\phi_1 \neq \phi_1'$, $\psi_1= \psi_2$, and that the metric is induced by a norm, we have
\begin{align*}
  & d([\psi_1, \psi_2] \cdot \langle \phi_1, \phi_2 \rangle, [\psi_1, \psi_2] \cdot \langle \phi_1', \phi_2' \rangle ) \\
  & = d(\psi_1 \cdot \phi_1 + \psi_2 \cdot \phi_2, \psi_1 \cdot \phi_1' + \psi_2 \cdot \phi_2') \\
...
Consequently, we postulate the following axiom:
  ...
  \begin{example}[Random walk]
    ...
    The following is useful to compute the total variation norm between measures \cite{tsybakov2008}. Let  let $P$ and $Q$ be two probability measures on $\mathcal{M}\mathbb{R}$. Define
    \[
    ...
    \todo[inline]{DUVIDA:Não temos de provar que a norma dos numeros reais tb satisfaz as condições}
...
Subsequently, let $\mu =\emph{u}_{a,b}(*)$ and  $\mu'=\emph{u}_{c,d}(*)$. Then, the Radon-Nikodym derivatives (probability density functions) with respect to \( \nu = U(a, b) + U(c, d) \) are given by:
...
Here, we calculate
\begin{align*}
  \norm{U(a,b)-U(c,d)} = \int |u(a,b)(x)- u(c,d)(x)| \, d\nu(x) = \int |u(a,b)(x)- u(c,d)(x)| \, dx 
\end{align*}
...
We compute,
\begin{align*}
  \norm{\delta_{x_0}-U(x_0 - \epsilon, x_0 + \epsilon)} &= \int |f(x)- u(x_0 - \epsilon, x_0 + \epsilon) (x)| \, d\nu(x) \\
  &= \int |f(x)| \, dx + \int |u(x_0 - \epsilon, x_0 + \epsilon)(x)| \, dx = 2
\end{align*}
...
\begin{proposition} \cite[Proposition 3.2.3]{effrosOperatorSpaces2000} \label{prop:cb_dual}
...
\todo[inline]{$\odot$ -> tensor algebrico}

\begin{proposition} \label{prop:cb_tensores}
  Given normal maps $ \phi \,\widecheck{\otimes}_\varepsilon \psi: B(\mathcal{H}_1) \,\widecheck{\otimes}_\varepsilon B(\mathcal{H}_2)  \to  B(\mathcal{K}_1) \,\widecheck{\otimes}_\varepsilon B(\mathcal{K}_2)$ and $ \phi \, \bar{\otimes} \, \psi: B(\mathcal{H}_1) \, \bar{\otimes} \, B(\mathcal{H}_2)  \to  B(\mathcal{K}_1) \, \bar{\otimes} \, B(\mathcal{K}_2)$, it holds that
  \[
...
\begin{proof}
  Let $\phi :  B(\mathcal{H}_1) \to  B(\mathcal{H}_2)$ and $\psi : B(\mathcal{K}_1) \to (\mathcal{K}_2)$ be normal maps. Since $ f $ and $g$ are normal, $\phi_* \odot_\varepsilon \phi_*$ can be  viewed  as a
  restriction of $(\phi \odot_\varepsilon \psi)^*$ \cite{Cho2014}. Consequently, it follows that $ \cbnorm{\phi_* \odot_\varepsilon \phi^*} \leq \cbnorm{(\phi \odot_\varepsilon \psi)_*}$ and therefore, $ \cbnorm{\phi^*  \,\widecheck{\otimes}_\varepsilon \phi^*} \leq \cbnorm{(\phi  \,\widecheck{\otimes}_\varepsilon \psi)^*}$. As a result, attending to Proposition \ref{prop:cb_dual} it holds that 

  \begin{align*}
    \cbnorm{\phi \,\bar{\otimes} \, \psi} = \cbnorm{(\phi_* \,\widecheck{\otimes}_\varepsilon \psi_*)^*} = \cbnorm{\phi_* \,\widecheck{\otimes}_\varepsilon \psi_*} \leq \cbnorm{(\phi \,\widecheck{\otimes}_\varepsilon \psi)^*} = \cbnorm{\phi \,\widecheck{\otimes}_\varepsilon \psi}
  \end{align*}

\end{proof}

\todo[inline]{WstarCPSU -> is the category of bounded operators in a hilbert space (B(H)) and normal completely positive subunital maps.}
...
Next, regarding $\bar{\otimes}$, the inequations $\cbnorm{\phi \, \bar{\otimes} \id}  \leq \cbnorm{\phi}$ and  $ \cbnorm{\id \, \bar{\otimes}  \phi} \leq \cbnorm{\phi}$ are  sufficient conditions to prove the second part of the proposition. There inequations follow form Propositions \ref{prop:eqs_id_cb} and \ref{prop:cb_tensores}.
\end{proof}

\todo[inline]{A norma da soma direta é o máximo -> tese kenta sho}
