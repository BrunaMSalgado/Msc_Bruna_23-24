...
\begin{tikzpicture}
  \matrix (m) [matrix of math nodes,row sep=4em,column sep=7em,minimum width=2em]
  {
    \left([\![ \Delta_{1},\typeA,\typeB, \Delta_{2} ]\!]\right)  \otimes  (\typeD \oplus \typeE)  &   \left([\![ \Delta_{1},\typeB,\typeA, \Delta_{2} ]\!]\right)  \otimes  (\typeD \oplus \typeE)   \\
  ...
  \path[-stealth]
    ...
    (m-2-1)  edge[bend right=-10] node [pos=0.5, shift={(-1.3, 0)}] {$[\text{id} \otimes\inl,\text{id} \otimes\inr]$} (m-1-1)
    ...
    (m-2-2)  edge[bend right=-10] node [pos=0.5, shift={(-1.3, 0)}] {$[\text{id} \otimes\inl,\text{id} \otimes\inr]$} (m-1-2)
    ;
\end{tikzpicture}
