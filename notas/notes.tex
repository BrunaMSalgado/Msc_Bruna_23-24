...
%\]
%This space consists of matrix tuples $A = (A_1, \dots, A_s)$, where the signature $\sigma$ specifies both the number $s$ of matrices and their respective dimensions $n_i \times n_i$. The elements of $V_\sigma$ are represented uppercase letters such as $A$, $B$, etc.
\todo[inline]{Colocar disclaimer a dizer que em embora os $A's$ sejam vetores verticais qd daí não advir nenhuma ambiguidade vamos representar como pares por uma questão de ficar mais simples}

Here, the concepts of trace, and matrix multiplication are extended to vectors of matrices $A=(A_1, \ldots, A_n) , U = (U_1, \ldots, U_n), U' = (U'_1, \ldots, U'_n)  \in \mathcal{M}_{\sigma}$ as follows:
\begin{align*}
  &\tr(A) = \sum_{i} \tr(A_{i}) \\
  %&(A)^{\dagger} = (A_{1}^{\dagger}, \ldots, A_{n}^{\dagger}) \\
  & U A U' = (U_1 A_{1} U'_1, \ldots, U_n A_{n} U'_n)
\end{align*}
...
\end{definition}
for positive $A \in \mathcal{M}_{n}.$
...
\begin{definition}  \label{def:gen_1norm}  \emph{(Generalized) Trace norm of superoperators}
  Let $\phi \in \mathcal{M}_{\sigma}\rightarrow \mathcal{M}_{\tau}$ be a superoperator, where $\sigma =(n_1, \ldots, n_s)$ and $\tau =(m_1, \ldots, m_t)$.  Defining  $\phi_i= (\phi_{i1}, \ldots, \phi_{it})$ for $n_i \in \sigma$, the norm of $\phi$ is defined as
...
\begin{align*}
  ...
  &  \sup_{i,j} \left\{  \lVert \phi_{i} (A_i) \rVert_{L^1},   \lVert \phi_{j} (B_j) \rVert_{L^1}  \, \vert \, \lVert A_i \rVert= 1 ,  \lVert B_j \rVert = 1 \right\} \\
\end{align*}


Given that the norm is continuos and compact, we can use the maximum instead of the supremum. Considering $A_{11}, \ldots, A_{1s}, A_{21}, \ldots, A_{2t} $ as the matrices that maximize the left-hand side of the inequality, if the inequality bellow is proven so it the equality above:

\begin{align*}
  & \sum_{i,j} \left\lVert \phi_{i} (A_{1i})  \right\rVert_{L^1} 
  \leq  \max_{\phi_i,\psi_j} \left\{  \lVert \phi_{i} (A_{1i} / \lVert A_{1i} \rVert ) \rVert_{L^1},   \lVert \psi_{i} (A_{2j} / \lVert A_{2j} \rVert ) \rVert_{L^1}  \right\}  
  \wedge \sum_{i,j} \lVert A_{1i} \rVert+ \lVert A_{1j} \rVert=1 
\end{align*}


 Proving the inequality above is equivalent to demonstrating that for all $a_1, \ldots, a_n, x_1, \ldots, x_n \in \mathbb{R}^{+}_{0}$ with $a_1+ \ldots + a_n=1$,
\begin{align*}
...
It remains to prove the reverse inequality. The validity of this inequality follows from the observation that
\begin{align*}
...
\begin{lemma} \label{lem_op_max_diamond}
  Let $\sigma: n_1, \ldots, n_s$, $\tau: m_1, \ldots, m_t$ and $\mu: o_1, \ldots, o_r$ be signatures. For all superoperators $\phi \in \catQ(\sigma,\mu)  $ and $\psi \in \catQ(\tau,\mu) $ it holds that 
...
\begin{proposition} \label{prop:uv*}
  Let $\sigma: n_1, \ldots, n_s$, $\tau: m_1, \ldots, m_t$ and $\mu: o_1, \ldots, o_r$ be signatures. For all super-operators $\psi: \sigma \to \tau$, it holds that:
  \begin{align*}
    \gentracenorm{\phi} & =  \sup_{i}  \left\{ \sup \left\{ \sum_{j} \|\phi_{ij} (v_i w_i^\dag)\|_{1}  \mid  \|v_i\|_{2} = 1,   \|w_i\|_{2} =1 \right\} \right\}
  \end{align*}
\end{proposition}
...
\begin{proof}
  ...
  Every square matrix $A \in  \mathcal{M}_n $ that satisfies $ \tracenorm{A} \leq 1$ can be expressed as a convex combination of operators of the form  $uv*$ where $u,v  \in \mathbb{C}^{n}$ are unit vectors. Consequently by  triangle inequality, one has that,

  \begin{align*}
    &\gentracenorm{\phi} = \sup_{i}  \left\{ \sup \left\{ \sum_{j} \|\phi_{ij} (A_i)\|_{1}  \mid  \|A_i\|_{1} = 1 \right\} \right\} \\
    & = \sup_{i}  \left\{ \sup \left\{ \sum_{j} \left\lVert \phi_{ij} \left( \sum_k v_i w_i^\dagger \right) \right\rVert_{1}   \Biggm| \sum_k \left\lVert v_k \right\rVert_{2} = 1,  \sum_k \left\lVert w_k \right\rVert_{2} = 1  \right\} \right\} \\
    & \leq  \sup_{i}  \left\{ \sup \left\{ \sum_{j} \sum_k  \left\lVert \phi_{ij} \left(  v_k w_k^\dagger \right) \right\rVert_{1}   \Biggm|\sum_k \left\lVert v_k \right\rVert_{2} = 1,  \sum_k \left\lVert w_k \right\rVert_{2} = 1   \right\} \right\}
  \end{align*}

  Since, for each $\phi_{ij} \left(  v_k w_k^\dagger \right)$, there exists a $vw^\dagger$ such that  $\phi_{ij} \left(  v_k w_k^\dagger \right) \leq \phi_{ij} \left(  v w^\dagger \right)$ for all $k$, the inequality above can be rewritten as:
  \begin{align*}
    & \sup_{i}  \left\{ \sup \left\{ \sum_{j}  \left\lVert \phi_{ij} \left(  v_i w_i^\dagger \right) \right\rVert_{1}   \Biggm|    \|v_i\|_{2} = 1,   \|w_i\|_{2} \right\} \right\}
  \end{align*}

  The proof of the reverse inequality follows a similar argument to that of the analogous inequality in Lemma \ref{lem_op_max_trace}
  
  \begin{comment}
  The validity of this inequality follows from the observation that
\begin{align*}
  & \left\{ \sum_{j}  \left\lVert \phi_{ij} \left(  v_i w_i^\dagger \right) \right\rVert_{1}   \Biggm|    \|v_i\|_{2} = 1,   \|w_i\|_{2} \right\}  \subseteq\\
&  \left\{ \sum_{j} \left\lVert \phi_{ij} \left( \sum_k v_i w_i^\dagger \right) \right\rVert_{1}   \Biggm| \sum_k \left\lVert v_k \right\rVert_{2} = 1,  \sum_k \left\lVert w_k \right\rVert_{2} = 1  \right\},
\end{align*}
This inclusion holds because the first set corresponds to the special case where all but one element in the sum over $k$ are zero. Consequently
\begin{align*}
  & \sup_{i}  \left\{ \sup \left\{ \sum_{j}  \left\lVert \phi_{ij} \left(  v_i w_i^\dagger \right) \right\rVert_{1}   \Biggm|    \|v_i\|_{2} = 1,   \|w_i\|_{2} \right\} \right\} \\
  &\leq \sup_{i}  \left\{ \sup \left\{ \sum_{j} \left\lVert \phi_{ij} \left( \sum_k v_i w_i^\dagger \right) \right\rVert_{1}   \Biggm| \sum_k \left\lVert v_k \right\rVert_{2} = 1,  \sum_k \left\lVert w_k \right\rVert_{2} = 1  \right\} \right\}
\end{align*}
and the equality is proven.
  \end{comment}
\end{proof}
...
\begin{lemma} \label{lemma:uv*_tensorI_gen}
  Let $\sigma: n_1, \ldots, n_s$ be a signature and $\phi_i: \mathcal{M}_{n}  \rightarrow \sigma$ be a super-operator.  For every choice of a complex vector space $\mathbb{C}^{m}$ and unit vectors $v_1, v_2 \in \mathbb{C}^{n} \otimes \mathbb{C}^{m}$, there exist unit vectors $w_1, w_2 \in \mathbb{C}^{n} \otimes \mathbb{C}^{n}$ such that the following equalities hold:
  \begin{align*} 
       \sum_{j=1}^{m} \left\lVert   \phi_{ij} \otimes \id_{\mathcal{M}_{m}} (v_1 v_2^\dag)\right\rVert_{1} = \sum_{j=1}^{m} \left\lVert \phi_{ij}  \otimes \id_{\mathcal{M}_{n}} (w_1 w_2^\dag) \right\rVert_{1}
  \end{align*}
\end{lemma}


 \begin{proof}
  The proof of this Lemma follows from the one presented in \cite[Proof of Lemma 3.45]{watrous2018theory}. Since the vectors $w_1$ and $w_2$ satisfying the inequality
  \begin{align*}
  \lVert \phi_{ij} \otimes \id_{\mathcal{M}{m}} (v_1 v_2^\dag) \rVert{1} = \lVert \phi_{ij} \otimes \id_{\mathcal{M}{n}} (w_1 w_2^\dag) \rVert{1}
  \end{align*}
  depend only on $v_1$ and $v_2$ and are independent of $\phi_{ij}$ , it follows that the equality in Lemma \ref{lemma:uv*_tensorI_gen} holds.
  In the case that $m \leq n$ for any choice of an isometry $T \in \mathbb{C}^{m} \rightarrow \mathbb{C}^{n} $, the vectors $w_1 = (\id_{\mathbb{C}^{n}} \otimes T)v_1$ and $w_2 = (\id_{\mathbb{C}^{n}} \otimes T)v_2$  satisfy the equality above. If  $m > n$, consider Schmidt decompositions
  \begin{align*}
      v_1 = \sum_{k=1}^n \sqrt{p_k} \, v_{1k} \otimes x_k \quad \text{and} \quad v_2 = \sum_{k=1}^n \sqrt{q_k} \, v_{2k} \otimes y_k 
  \end{align*}
  of $v_1$ and $v_2$, a suitable choice for the vectors $w_1$ and $w_2$ is given by
  \begin{align*}
      u = \sum_{k=1}^n \sqrt{p_k} \, v_{1k} \otimes v_{1k} \quad \text{and} \quad v = \sum_{k=1}^n \sqrt{q_k} \, v_{2k} \otimes v_{2k}. 
  \end{align*}
 \end{proof}

 \todo[inline]{Escrevi tudo, para o caso do lemma da op norm sair. Mas se não sair a prova disto é só uma frase, dado que as provas são basicamente iguais}

 \begin{lemma}\label{lem:phi(A)}
  Let $\sigma: n_1, \ldots, n_s$, $\tau: m_1, \ldots, m_t$ and $\mu: o_1, \ldots, o_r$ be signatures. Let  $\phi: \sigma \rightarrow \mu$ be a super-operator, then for $A \in \sigma$ it holds that:
  \begin{align*}
    \gentracenorm{\phi(A)} \leq \gentracenorm{\phi} \cdot \lonenorm{A}.
  \end{align*}
\end{lemma}

\begin{proof}

The generalized trace norm of $\phi$ corresponds to :
\begin{align*}\label{eq:q}
  \gentracenorm{\phi}& =   \sup_i \left\{ \sup \left\{\|\phi_{i} (A_i)\|_{L^1}   \mid \hspace{1pt} \|A_i\|_{1} = 1 \right\} \right\} & \{\text{\autoref{def:gen_1norm}}\} \\
 & = \sup_i  \left\{\|\phi_{i} (A_i)\|_{L^1}   \mid \hspace{1pt} \|A_i\|_{1} = 1 \right\} 
\end{align*}


To prove that  if  $ \lonenorm{A} = 1$
\begin{align*} 
  \lonenorm{\phi(A) } \leq \gentracenorm{\phi},
\end{align*}
is equivalent to demonstrating that,
\begin{align*}
 &\sup \left\{ \lVert \phi(A) \rVert_{L^1} \hspace{2pt}  \Biggm| \hspace{2pt}  \sum_{i=1}^{n} \lVert A_i  \rVert_1 = 1 \right\} \leq \gentracenorm{\phi} \\
 & \Leftrightarrow \sup \left\{ \left\lVert \sum_{i=1}^{n} \phi_{i} (A_{i}) \right\rVert_{L^1} \hspace{2pt}  \Biggm| \hspace{2pt}  \sum_{i=1}^{n} \lVert A_i  \rVert_1 = 1  \right\} 
 \leq \sup_i  \left\{\|\phi_{i} (A_i)\|_{L^1}   \mid \hspace{1pt} \|A_i\|_{1} = 1 \right\} 
\end{align*}


By the triangle inequality, if the inequality bellow holds, then the inequality above also holds.
\begin{align*}
    &\sup \left\{   \sum_{i=1}^{n} \lVert \phi_{i} (A_{i}) \rVert_{L^1} \hspace{2pt}  \Biggm| \hspace{2pt}  \sum_{i=1}^{n} \lVert A_i  \rVert_1 = 1 \right\} 
    \leq \sup_i  \left\{\|\phi_{i} (A_i)\|_{L^1}   \mid \hspace{1pt} \|A_i\|_{1} = 1 \right\}   \\
\end{align*}

Given that the norm is continuos and compact, we can use the maximum instead of the supremum. Considering $A_1, \ldots, A_n$ as the matrices that maximize the left-hand side of the inequality, proving the following inequality is equivalent to demonstrating that inequality above holds.
  \begin{align*}
     &\max \left\{ \sum_{i=1}^{n} \lVert \phi_{i} (A_{i}) \rVert_{L^1} \right\} 
     \leq \max_{\phi_i} \left\{ \lVert \phi_{i} (A_{i} / \lVert A_{i} \rVert_1) \rVert_{L^1} \right\} 
     \wedge \sum_{i=1}^{n} \lVert A_i  \rVert_1 = 1
  \end{align*}
  ...
  Designating $M = \max \left\{   \dfrac{1}{a_1} x_1  , \ldots , \dfrac{1}{a_n} x_n   \right\}$, from the definition of maximum it follows that, for all $1 \leq i \leq n$, $x_i \leq M \cdot a_i$, and consequently, $x_1 + \ldots + x_n \leq M \cdot (a_1 + \ldots + a_n) = M$. 
  
  Therefore, it follows that for an operator $A \in \sigma $,
  \begin{equation*}
    \left\lVert \phi \left(\frac{A}{\lVert A \rVert_{L^1}}\right)  \right\rVert_{L^1} = \frac{1}{\lVert A \rVert_{L^1}} \left\lVert \phi (A)  \right\rVert_{L^1}
  \end{equation*} 
is upper bounded by $\lVert \phi  \rVert_{L^1}$. Thus, Lemma \ref{lem:phi(A)} holds.

\end{proof}

 \begin{theorem} \label{theorem:tensor_stability} 
  Let $\sigma: n_1, \ldots, n_s$, $\tau: m_1, \ldots, m_t$ and $\mu: o_1, \ldots, o_r$ be signatures. For all super-operators $\phi: \sigma  \rightarrow \tau$ and signatures $\mu$,  it holds that:
      \begin{align*}
          \gentracenorm{\phi \otimes \id_{\mu}} \leq  \gendiamondnorm{\phi}
      \end{align*}
    with equality holding under the assumption that within $\mu$ there  exists a natural number $o_i$, such that for all $1 \leq j \leq n$, $\text o_i \geq n_j$.
  ...
  \begin{proof}
   This proof follows the same principle as \cite[Proof of Theorem 3.36]{watrous2018theory}.

    Attending to definitions \ref{def:gen_diamond_norm}, \ref{def:gen_1norm}, and \ref{def:tensor} , one has that
    \begin{align*} \label{eq:q_diamondgen_extendido}
        \lVert \phi \rVert_{\diamondsuit \text{ gen}}  & = \gentracenorm{Q \otimes \id_{\mu}} \\
        & = \sup_{i,k} \left\{ \sup \left\{ \sum_{j=1}^{t} \|\phi_{ij} \otimes \id_{\mathcal{M}_{o_k}} (A_{ik})\|_{1}    \mid  \|A_{ik}\|_{1} = 1 \right\} \right\}  
    \end{align*}

    When $i \neq k$, considering Proposition \autoref{prop:uv*} and Lemma \autoref{lemma:uv*_tensorI_gen}, considering unit vectors  $v_{ik}, w_{ik} \in \mathbb{C}^{n_i} \otimes \mathbb{C}^{n_j}$, and $v_{i}, w_{i} \in \mathbb{C}^{n_i} \otimes \mathbb{C}^{n_i}$, where $1 \leq i \leq s$ and $1 \leq k \leq s$, it follows that
    \begin{align*}
      & \sup_{i,k} \left\{ \sup \left\{ \sum_{j=1}^{t} \|\phi_{ij} \otimes \id_{\mathcal{M}_{o_k}} (A_{ik})\|_{1}    \mid  \|A_{ik}\|_{1} = 1 \right\} \right\} \\
      & = \sup_{i,k} \left\{ \sup \left\{\sum_{j=1}^{t} \|\phi_{ij} \otimes \id_{\mathcal{M}_{o_k}} (v_{ik} w_{ik}^\dag )\|_{1} \right\}   \right\} \\
      & \leq \sup_{i} \left\{ \sup \left\{ \sum_{j=1}^{t} \|\phi_{ij} \otimes \id_{\mathcal{M}_{o_i}} (v_{i} w_{i}^\dag )\|_{1}   \right\} \right\}.
    \end{align*}
  
  Consequentlty, 
  \begin{align*}
    \lVert \phi \rVert_{\diamondsuit \text{ gen}} = \sup_{i} \left\{ \sup \left\{ \sum_{j=1}^{t} \|\phi_{ij} \otimes \id_{\mathcal{M}_{o_i}} (A_{i})\|_{1}    \mid  \|A_{i}\|_{1} = 1 \right\} \right\}
  \end{align*}
  
   
    Applying the same reasoning to  $\gentracenorm{\phi \otimes I_{\mu}}$,  the following inequality holds:
  \begin{align*}
      \gentracenorm{\phi \otimes I_{\mu}}  & \leq \sup_{i} \left\{ \sup \left\{ \sum_{j=1}^{t} \|\phi_{ij} \otimes \id_{\mathcal{M}_{o_i}} (v_{i} w_{i}^\dag )\|_{1}   \right\} \right\}.
  \end{align*}
  As a result,
  $$\gentracenorm{Q \otimes \id_{\mu}} \leq  \gendiamondnorm{Q}$$
  
  
   Consider $1 \leq i \leq s$ and $1 \leq k \leq r$. When $o_k \geq n_i$, there exists an isometry $T: \mathbb{C}^{n_i} \rightarrow \mathbb{C}^{o_k}$. Moreover, for $A \in \mathcal{M}_{n_i}$, such that $\|A\|_1=1$, and $\phi_{ij}: \mathcal{M}_{n_i} \rightarrow \mathcal{M}_{o_k}$, attending to the isometric invariance of the trace norm it holds that:
    \begin{equation*}
      \sum_{j=1}^{m} \lVert \phi_{ij} \otimes \id_{\mathcal{M}_{n_i}} (A) \rVert_{1} = \sum_j \lVert (\phi_{ij} \otimes \id_{\mathcal{M}_{o_k}}) ((\id_{\mathcal{M}_{n_i}} \otimes T) A (\id_{\mathcal{M}_{n_i}} \otimes T )^\dag) \rVert_{1}. 
    \end{equation*}
  This can be rewritten as:
  \begin{align*}
    &\sum_{j=1}^{m} \lVert \phi_{ij} \otimes \id_{\mathcal{M}_{n_i}} (A) \rVert_{1} =  \lonenorm{(\phi_{i} \otimes \id_{\mathcal{M}_{o_k}}) ((\id_{\mathcal{M}_{n_i}} \otimes T) A (\id_{\mathcal{M}_{n_i}} \otimes T )^\dag)} . 
  \end{align*}
  Consequentlty, by Lemma \ref{lem:phi(A)}, it follows that:
  \begin{equation*}
    \sum_{j=1}^{m} \lVert \phi_{ij} \otimes \id_{\mathcal{M}_{n_i}} (A) \rVert_{1} \leq \lonenorm{\phi_{i} \otimes \id_{\mathcal{M}_{o_k}}} = \sup \left\{\sum_{j=1}^{m} \lVert \phi_{ij} \otimes \id_{\mathcal{M}_{o_k}} (B) \rVert_{1} \mid \|B\| = 1\right\}.
  \end{equation*}
  And, as a result,  under the assumption that within $\mu$ there  exists a natural number $o_i$, such that for all $1 \leq j \leq n$, $\text o_i \geq n_j$, it holds that: 
  \begin{equation*}
    \gentracenorm{\phi \otimes \id_{\mathcal{M}_\mu}} =  \gendiamondnorm{\phi}
  \end{equation*}
  ...
  \begin{corollary} \label{cor:tensor_stability}
    For all super-operators  $\phi: \sigma  \rightarrow \tau$ and signatures $\mu$  it holds that:
  \begin{align*}
    \gendiamondnorm{\phi \otimes \id_{\mathcal{M}_\mu}} =  \gendiamondnorm{\phi}
\end{align*}
  \end{corollary}


  \begin{proposition}\label{prop:gen_trace_submultiplicative}
    Let $\sigma: n_1, \ldots, n_s$, $\tau: m_1, \ldots, m_t$ and $\mu: o_1, \ldots, o_r$ be signatures.  The generalized trace norm is submultiplicative with respect to composition of super‑operators, \textit{i.e.}, or all super-operators $\phi: \sigma  \rightarrow \tau$ and $\psi: \tau  \rightarrow \mu$, it holds that:
    \begin{align*} 
      \gentracenorm{\psi  \phi} \leq \gentracenorm{\psi} \gentracenorm{\phi}
    \end{align*}
  \end{proposition}
  
  \begin{proof}
    This proof follows the same reasoning as \cite[Proof of Proposition 3.41]{watrous2018theory}. 
  
   The composition $\psi \cdot \phi$ can be seen as:
  \begin{align*}
    \psi \cdot \phi = 
    \begin{pmatrix}
      \psi_{11} & \ldots & \psi_{t1}, \\
      \vdots & \ddots  & \vdots \\
      \psi_{1r} & \ldots & \psi_{tr} \\
    \end{pmatrix}
    \cdot
    \begin{pmatrix}
      \phi_{11} & \ldots & \phi_{s1}, \\
      \vdots & \ddots  & \vdots \\
      \phi_{1t} & \ldots & \phi_{st} \\
    \end{pmatrix}
    =\begin{pmatrix}
      \psi \cdot \phi_1, \ldots, \psi \cdot \phi_n .\\
    \end{pmatrix}
    \end{align*}
  
 %Defining  $\phi_i= (\phi_{i1}, \ldots, \phi_{it})$ for $i \in \sigma$, the norm of $\phi$ is defined as 

  Attending to Definition \autoref{def:gen_1norm}, it follows that:
  \begin{align*} 
   \gentracenorm{\psi \cdot  \phi} & = \sup_{i} \{ \sup \{\lVert \psi \cdot \phi_i (A_i) \rVert_{L^1} \mid \|A_i\|_{1} = 1\} \}  = \sup_i \{\lVert \psi \cdot \phi_i (A_i) \rVert_{L^1} \mid \|A_i\|_{1} = 1\}
  \end{align*}

  Let $B \in \tau$, by \autoref{lem:phi(A)} it follows that:
  \begin{align*}
    \lVert \psi(B) \rVert_{L^1} \leq \lVert \psi  \rVert_{L^\infty} \cdot \lVert B  \rVert_{L^1}.
  \end{align*}
  
  As a result, 
  \begin{align*} 
    \lVert \psi (\phi_i (A_i)) \rVert_{L^1} \leq \lVert \psi  \rVert_{L^\infty} \cdot \lVert \phi_i (A_i)  \rVert_{L^1},
  \end{align*}

  Considering Definition \autoref{def:gen_1norm}, $\lVert \phi_i (A_i)  \rVert_{L^1}\leq \gentracenorm{\phi}$. Therefore, one has that,
  \begin{align*} 
    \lVert \psi (\phi_i (A_i)) \rVert_{L^1} \leq \lVert \psi  \rVert_{L^\infty} \cdot \lVert \phi  \rVert_{L^\infty},
  \end{align*} 

  Finally considering the definition of \textit{supremum}, it follows that,
  \begin{align*} 
    \lVert \psi \rVert_{L^\infty} \leq \lVert \psi  \rVert_{L^\infty} \cdot \lVert \phi  \rVert_{L^\infty},
  \end{align*}
  ...
  \begin{proposition}\label{prop:gen_diamond_submultiplicative}
    Let $\sigma: n_1, \ldots, n_s$, $\tau: m_1, \ldots, m_t$ and $\mu: o_1, \ldots, o_r$ be signatures. The generalized diamond norm is submultiplicative with respect to composition of super‑operators, \textit{i.e.}, for all super-operators $\phi: \sigma  \rightarrow \tau$ and $\psi: \tau  \rightarrow \sigma$, it holds that:
    \begin{equation*}
      \gendiamondnorm{\psi  \phi} \leq  \gendiamondnorm{\psi} \gendiamondnorm{\phi}
    \end{equation*}
  \end{proposition}
  
  \begin{proof}
   
    By Lemma \ref{lem:gen_trace_submultiplicative}, it is possible to state that:
  \begin{equation*}
     \gentracenorm{\psi \phi \otimes \id_{\sigma}} \leq \gentracenorm{\psi \otimes \id_{\sigma}} \cdot \gentracenorm{\phi \otimes \id_{\sigma}}
  \end{equation*}
  Attending to Definition \ref{def:gen_diamond_norm}, it follows that:
  \begin{align*}
     \gendiamondnorm{\psi \phi} \leq  \gentracenorm{ \psi \otimes \id_{\sigma} } \cdot \gendiamondnorm{ \phi}
  \end{align*}
  Given Theorem \ref{theorem:tensor_stability}, it holds that
  \begin{equation*}
    \gentracenorm{\psi \otimes \id_{\sigma}} \leq \gendiamondnorm{\psi}
  \end{equation*}
  As a result, the inequality in Proposition \ref{prop:gen_diamond_submultiplicative} holds.
  \end{proof}

  \begin{theorem} \label{thm:gen_trace_cptp_norm}
    Let $\sigma: n_1, \ldots, n_s$ and  $\tau: m_1, \ldots, m_t$  be signatures. Let  $\phi: \sigma  \rightarrow \tau$ be a  positive trace-nonincreasing super-operator. Then, it holds that $\gentracenorm{\phi} \leq 1$.
  \end{theorem}

  \begin{proof}
    Let $\phi_i= (\phi_{i1}, \ldots, \phi_{it})$ for $n_i \in \sigma$. Considering that for a positive superoperator $\phi_{ij}:\mathcal{M}_{n} \to \mathcal{M}_{m} $ and positive operator $A_{i}: \mathcal{M}_{n}$ it holds that
    \begin{align*}
    \tracenorm{\phi_{ij} (A_{i})} & = \tr\left(\sqrt{\left(\phi_{ij} (A_{i})\right)^\dag (\phi_{ij} (A_{i}))}\right) \\
    & = \tr\left(\sqrt{\left(\phi_{ij} (A_{i})\right) (\phi_{ij} (A_{i}))}\right) & (\phi_{ij} (A_{i}) \text{ is hermitian}) \\
    & = \tr\left(\phi_{ij} (A_{i})\right)
    \end{align*}
    it follows that, for unitary matrices $U_j$,
    \begin{align*}
       \lonenorm{\phi_{i}} & = \sup \left\{ \sum_{j} \tracenorm{(\phi_{ij}(A_i))} \mid \tracenorm{A_i} = 1 \right\} & (\text{Def. } \ref{def:gen_1norm_prod} ) \\
      & = \sup \left\{ \sum_{j} \left\lvert \langle U_j, \phi_{ij}(A_i)  \right\rvert  \rangle   \mid \tracenorm{A_i} =1,  \right\} & (\text{Def. trace norm for square matrices}) \\
      & \leq  \sup \left\{ \sum_{j} \tracenorm{\phi_{ij}(A_i)} \spectralnorm{U_j}  \mid \tracenorm{A_i} =1,  \right\} & (\text{H\"{o}lder inequality})\\
      & =\sup \left\{ \sum_{j} \tr\left(\phi_{ij} (A_{i})\right)  \mid \tracenorm{A_i} =1,  \right\} & (U_j \text{ is an isometry}) \\
      & \leq 1. & (\phi \text{ is trace-nonincreasing, Def. \ref{def:catQ}})
      \end{align*}
 
      As a result,
      \begin{align*}
      \gentracenorm{\phi} =  \sup_{\phi_i} \{ \sup \{\lVert \phi_i (A_i) \rVert_{L^1} \mid \|A_i\|_{1} = 1\} \} \leq 1
      \end{align*}

  \end{proof}

 
  \todo[inline]{$||\cdot||_{\infty}$ -> spectral norm}
  \todo[inline]{Substituir "Def. trace norm for square matrices" pela eq}


  \begin{corollary} \label{cor:gen_diamond_cptp_norm}
    Let $\sigma: n_1, \ldots, n_s$ and  $\tau: m_1, \ldots, m_t$  be signatures. Let  $\phi: \sigma  \rightarrow \tau$ be a  completely positive trace-nonincreasing super-operator. Then, it holds that $ \gendiamondnorm{\phi}\leq 1$.
  \end{corollary}

  \begin{proof}
    Given that $\phi$ is a  completely positive trace-nonincreasing super-operator, if follows that $ \phi \otimes \id_{\sigma}$ is a positive trace-preserving super-operator. As a result, it follows that $\gendiamondnorm{\phi}\leq 1$.
  \end{proof}

  \begin{proposition} \label{prop:gen_trace_subtraction}
    Let $\sigma: n_1, \ldots, n_s$, $\tau: m_1, \ldots, m_t$ and $\mu: o_1, \ldots, o_r$ be signatures. For all completely positive trace-nonincreasing super-operators $\phi_0, \phi_1: \sigma   \rightarrow \tau$ and $\psi_0, \psi_1: \tau \rightarrow \tau$, it holds that:
    \begin{equation*}
      \gentracenorm{\psi_0 \phi_0 - \psi_1\phi_1} \leq  \gentracenorm{ \phi_0- \phi_1 }  + \gentracenorm{\psi_0- \psi_1 }
    \end{equation*}
  \end{proposition}

  \begin{proof}
    The proof follows the same reasoning as in \cite[Proof of Proprosition 3.41]{watrous2018theory}, following from the triangle inequality, Lemma \ref{prop:gen_trace_submultiplicative} and Theorem \ref{thm:gen_trace_cptp_norm}. 

    \begin{align*}
      \gentracenorm{\psi_0 \phi_0 - \psi_1\phi_1 } & \leq \gentracenorm{\psi_0 \phi_0 - \psi_0 \phi_1 } + \gentracenorm{ \psi_0\phi_1  - \psi_1\phi_1} \\
      &=  \gentracenorm{\psi_0 (\phi_0 -\phi_1) } + \gentracenorm{(\psi_0- \psi_1) \phi_1} \\
      &\leq \gentracenorm{\psi_0 } \gentracenorm{\phi_0 -\phi_1 } + \gentracenorm{ \phi_1} \gentracenorm{\psi_0- \psi_1} \\
      &\leq \gentracenorm{ \phi_0- \phi_1 }  + \gentracenorm{\psi_0- \psi_1 }
      \end{align*}
  \end{proof}
  ...
  \begin{proposition} \label{prop:gen_diamond_subtraction}
    Let $\sigma: n_1, \ldots, n_s$, $\tau: m_1, \ldots, m_t$ and $\mu: o_1, \ldots, o_r$ be signatures. For all completely positive trace-nonincreasing super-operators $\phi_0, \phi_1: \sigma   \rightarrow \tau$ and $\psi_0, \psi_1: \tau \rightarrow \tau$, it holds that:
    \begin{equation*}
      \gendiamondnorm{\psi_0 \phi_0 - \psi_1\phi_1} \leq  \gendiamondnorm{ \phi_0- \phi_1 }  + \gendiamondnorm{\psi_0- \psi_1 }
    \end{equation*}
  \end{proposition}

  \begin{proof}
    The proof is analogous to the previous one, following from triangle inequality, Lemma \ref{prop:gen_diamond_submultiplicative} and Corollary \ref{cor:gen_diamond_cptp_norm}.
  \end{proof}

  \begin{proposition} \label{prop:gen_trace_matrix_multiplicative}
    Let $\sigma: n_1, \ldots, n_s$ and $\tau: m_1, \ldots, m_t$. For all $A \in \sigma$ and $B \in \tau$ it holds that:
    \begin{equation*}
      \lonenorm{A \otimes B} =\lonenorm{A} \lonenorm{B}
    \end{equation*}
  \end{proposition}
  

  \begin{proof}
    Consider the decompositions $A=(A_1, \ldots, A_s)$ and $B = (B_1, \ldots, B_t)$.
    Given that the trace norm of matrices is multiplicative with respect to tensor, it follows that
    \begin{align*}
        \lonenorm{A \otimes B} &=  \lonenorm{(A_1, \ldots, A_s) \otimes (B_1, \ldots, B_t) }  \\
      & = \lonenorm{(A_1 \otimes B_1, \ldots, A_1 \otimes B_t, \ldots, A_s \otimes B_1, \ldots, A_s \otimes B_t) } \\
      & = \sum_{i=1}^{s} \sum_{j=1}^{t} \lVert A_i \otimes B_j \rVert_1 = \sum_{i=1}^{n} \sum_{j=1}^{t} \lVert A_i \rVert_1  \lVert   B_j \rVert_1 = \sum_{i=1}^{n} \lVert A_i \rVert_1 \sum_{j=1}^{t}  \lVert   B_j \rVert_1 \\
      & = \lonenorm{A} \lonenorm{B}
  \end{align*}
\end{proof}

\begin{proposition} \label{prop:unitary_invariance_superops}
  Let $\sigma: n_1, \ldots, n_s$ and $\tau: m_1, \ldots, m_t$.
  Let $\psi: \sigma  \rightarrow \tau$ be a super-operator, let $U,U' \in \sigma$  and $V, V' \in \tau $ be unitary operators, and let $\phi, \psi: \sigma  \rightarrow \tau$ be defined as:
  \begin{equation*}
    \psi(A) = V \phi (U A U') V'
  \end{equation*}
  for all $A \in \sigma$. Then, it holds that $ \gentracenorm{\psi} = \gentracenorm{\phi}.$
\end{proposition}

\begin{proof}
  Let $\phi_i= (\phi_{i1}, \ldots, \phi_{it})$ and $\psi_i= (\phi_{i1}, \ldots, \phi_{it})$ for $n_i \in \sigma$, $m_j \in \sigma$ . Consider $U = (U_{1}, \ldots,U_{s})$, $U' = (U'_{1}, \ldots,U'_{s})$, $V = (V_{1}, \ldots,V_{t})$,$ V' = (V'_{1}, \ldots,V'_{t})$, and $A = (A_1, \ldots, A_n)$, it follows that:
  \begin{align*}
    \psi (A) & =    V \cdot \left(\phi(U_{1}A_1 U'_{1}, \hspace{2pt} \ldots \hspace{2pt}, U_{n} A_n U'_{n}) \right) \cdot V'\\
   &=  V \cdot \left(\sum_i^{n} \phi_i(U_{i}A_i U'_{i})  \right) \cdot V'  \\
    & =\sum_{i=1}^{n} \left( V_{1} \phi_{i1} (U_{i} A_i U'_{i} ) V'_{1}, \hspace{2pt} \ldots \hspace{2pt}, V_{2t} \phi_{it}(U'_{i}A_i U'_{1i}) V'_{t} \right) \\
    & = \sum_{i=1}^{n} \left( \psi_{i1}(A_i), \hspace{2pt} \ldots \hspace{2pt}, \psi_{im}(A_i) \right) =  \sum_{i=1}^{n} \psi_i (A_i)
  \end{align*}

Applying the same reasoning as in \cite[Proof of Proposition 3.41]{watrous2018theory},  by the unitary invariance of the trace norm and Lemma \ref{lem:phi(A)}, it follows that for $1 \leq i \leq n$: 
\begin{align*}
    \lonenorm{\psi_i (A_i)} & 
    =   \sum_{j=1}^m  \lVert V_{j} \phi_{i1} (U_{0} A_i U'_{i} ) V'_{j} \rVert_{1} \\
    &= \sum_{j=1}^m  \lVert \phi_{ij} (U_{i} A_i U'_{i}) \rVert_{1} \\
    &= \lonenorm{\phi_i (U_{i} A_i U'_{i})} \\
    & \leq  \lonenorm{\phi_i} \lVert A_i  \rVert_{1} 
\end{align*}
As a result, given Definition \ref{def:gen_1norm_prod} it follows that $ \lonenorm{\psi} \leq  \lonenorm{\phi}$ for all $1 \leq i \leq s$. Thus, attending to Definition \ref{def:gen_1norm}, it holds that,
\begin{equation*}
 \gentracenorm{\psi} \leq \gentracenorm{\phi}.
\end{equation*}

Given that $U, U', V, V'$ are unitary operators, it follows that, $\phi{ij} = V_{j}^\dag \psi_{ij}(U_{i}^\dag A U'^\dag_{i}) V'^\dag_{j}$, and consequently the same approach can be taken to find that $ \gentracenorm{\psi} \leq \gentracenorm{\phi} $, which concludes the proof.
\end{proof}

\begin{theorem}
  Let $\sigma: n_1, \ldots, n_s$, $\tau: m_1, \ldots, m_t$, $\mu: o_1, \ldots, o_r$ and $\nu: p_1, \ldots, p_u$ and be signatures.
  For all $\phi: \sigma  \rightarrow \tau$ and $\psi: \mu \rightarrow \nu$, it holds that:
  \begin{equation*}
    \gendiamondnorm{\phi \otimes \psi} =  \gendiamondnorm{\phi} \gendiamondnorm{\psi} 
  \end{equation*}
\end{theorem}

\begin{proof}
  This prove follows the same reasoning as \cite[Proof of Theorem 3.49]{watrous2018theory}. By Proposition \ref{prop:gen_diamond_submultiplicative} and Corollary \ref{cor:tensor_stability}, one has:
  \begin{align*}
      \gendiamondnorm{\phi \otimes \psi} & =  \gendiamondnorm{(\phi \otimes \id_{\mathcal{M}_\mu}) ( \id_{\mathcal{M}_\sigma} \otimes \phi )} \\
      & = \gendiamondnorm{\phi \otimes \id_{\mathcal{M}_\mu}} \gendiamondnorm{\id_{\mathcal{M}_\sigma} \otimes \psi}   \\
      & = \gendiamondnorm{\phi} \gendiamondnorm{\psi}
  \end{align*}

Consider operators $A \in \mathcal{M}_{\sigma \otimes \sigma}$ and $B \in \mathcal{M}_{\mu \otimes \mu} $ such that $\lonenorm{A}=1$ and $\lonenorm{B}= 1$, and such that these equalities hold:
\begin{align} \label{eq:equalities_Q_S}
    \gendiamondnorm{\phi} & = \lonenorm{\phi \otimes \id_{\mathcal{M}_\sigma}  (A)}\\
    \gendiamondnorm{\psi} & = \lonenorm{\phi \otimes \id_{\mathcal{M}_\mu}  (B)} \notag
\end{align}
Attending to Proposition \ref{prop:gen_trace_matrix_multiplicative}, it follows that: 

\begin{align} \label{eq:norm_o_times_p_equal_1}
    \lVert A \otimes B \rVert_{1 \text{ gen}} = \lVert A  \rVert_{1 \text{ gen}} \lVert B \rVert_{1 \text{ gen}} =1
\end{align}

Attending to Definition \ref{def:gen_diamond_norm} and Proposition \ref{prop:unitary_invariance_superops}, which implies that this norm
is invariant under permuting the ordering of tensor factors of super-operadores, one has
\begin{align*} \label{eq:q_otimes_s_gen_diamond_inv_u}
    \gendiamondnorm{\phi \otimes \psi} & = \gentracenorm{\phi \otimes \psi \otimes \id_{\mathcal{M}_{\sigma \otimes \mu}}}\\
    & = \gentracenorm{\phi \otimes \psi \otimes \id_{\mathcal{M}_\sigma} \otimes \id_{\mathcal{M}_\mu} } \\
    &= \gentracenorm{\phi \otimes \id_{\mathcal{M}_\sigma} \otimes \psi  \otimes \id_{\mathcal{M}_\mu} } \\
    & \geq  \lonenorm{\phi \otimes \id_{\mathcal{M}_\sigma} \otimes \psi  \otimes \id_{\mathcal{M}_\mu} (A \otimes B)} &(\text{Lemma \ref{lem:phi(A)} and  \autoref{eq:norm_o_times_p_equal_1}})
\end{align*}


By Proposition \autoref{prop:gen_trace_matrix_multiplicative} and \autoref{eq:equalities_Q_S}, it holds that:
\begin{align*}
    \gendiamondnorm{\phi \otimes \psi} 
    \geq & \lonenorm{\phi \otimes \id_{\mathcal{M}_\sigma} \otimes \psi  \otimes \id_{\mathcal{M}_\mu} (A \otimes B)} \\
   = & \lonenorm{\phi \otimes \id_{\mathcal{M}_\sigma} (A)} \lonenorm{\psi  \otimes \id_{\mathcal{M}_\mu} (B)}\\
   = &  \gendiamondnorm{\phi} \gendiamondnorm{\psi}
\end{align*}
which completes the proof.
\end{proof}
