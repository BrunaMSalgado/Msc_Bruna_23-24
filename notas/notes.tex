...
\begin{align*}
  ...
  & \hspace{10pt} \comp \text{sp}_{\Gamma, Z;\Delta} \comp \text{sh}_{E,Z}\\
  & = \left[[\![ w ]\!] ,[\![ u ]\!]\right] \comp (\text{jn}_{\Delta;\typeD}\comp \sw \oplus \text{jn}_{\Delta;\typeE } \comp \sw)  \comp \dist  \comp ([\![ v]\!] \otimes \text{id})  \comp (\text{jn}_{\Gamma; \typeA} \comp (\text{id} \otimes [\![  t ]\!] ) \comp \text{sp}_{\Gamma;Z} \otimes \text{id})   & {(\text{Coherence})} \\
  &  \hspace{10pt} \comp \text{sp}_{\Gamma,Z;\Delta} \comp \text{sh}_{E,Z} \comp \text{jn}_{E; Z}  \comp \text{sp}_{E;Z} \\
  & = \left[[\![ w ]\!] ,[\![ u ]\!]\right] \comp (\text{jn}_{\Delta;\typeD}\comp \sw \oplus \text{jn}_{\Delta;\typeE } \comp \sw)  \comp \dist  \comp ([\![ v]\!] \otimes \text{id}) \comp (\text{jn}_{\Gamma; \typeA} \comp \text{sp}_{\Gamma; \typeA} \otimes   \id) \comp \text{sp}_{\Gamma,\typeA;\Delta}   \comp \text{sh}_{E,\typeA}    & {(\text{Naturality})}   \\
  &  \hspace{10pt} \comp \text{jn}_{E; \typeA} \comp (\text{id} \otimes [\![ t ]\!] ) \comp \text{sp}_{E;Z} \\
  & = \left[[\![ w ]\!] ,[\![ u ]\!]\right] \comp (\text{jn}_{\Delta;\typeD}\comp \sw \oplus \text{jn}_{\Delta;\typeE } \comp \sw)  \comp \dist  \comp ([\![ v]\!] \otimes \text{id}) \comp \text{sp}_{\Gamma, \typeA;\Delta}  \comp \text{sh}_{E,\typeA}  \comp  \text{jn}_{E; \typeA} \comp (\text{id} \otimes [\![ t ]\!] )   & {(\text{Coherence})}   \\
...
\begin{align*}
  ...
  & =  \left[[\![w ]\!] ,[\![ u]\!] \right] \comp  (\text{jn}_{ \Delta, \typeA;\typeD} \comp \sw \oplus \text{jn}_{ \Delta,\typeA;\typeE} \comp \sw)  \comp \dist \comp  ([\![v]\!] \otimes \text{id}) \comp \text{sp}_{\Gamma; \Delta, Z} \comp ( \text{id} \otimes     & {(\text{coherence and}} \\
  &  \hspace{10pt} (\text{jn}_{\Delta; \typeA}  \comp  (\text{id} \otimes [\![t]\!]) \cdot \text{sp}_{\Delta; Z} ) ) \comp \text{sp}_{\Gamma;\Delta,Z} \comp \text{sh}_{E,Z} \comp \text{jn}_{E; Z}  \comp \text{sp}_{E;Z} &  {\text{bifunctoriality of } \otimes})\\
  & = \left[[\![w ]\!] ,[\![ u]\!] \right] \comp  (\text{jn}_{ \Delta, \typeA;\typeD} \comp \sw \oplus \text{jn}_{ \Delta,\typeA;\typeE} \comp \sw)  \comp \dist \comp  ([\![v]\!] \otimes \text{id})  \comp ( \id \otimes \text{jn}_{\Delta; \typeA} \comp \text{sp}_{\Delta; \typeA})  & {(\text{naturality})}   \\
  &  \hspace{10pt} \comp \text{sp}_{\Gamma;\Delta,\typeA}   \comp \text{sh}_{E,\typeA}    \comp \text{jn}_{E; \typeA} \comp (\text{id} \otimes [\![ t ]\!] ) \comp \text{sp}_{E;Z} \\
  & =  [[\![w]\!], [\![u]\!]] \comp (\text{jn}_{ \Delta, \typeA;\typeD} \comp \sw \oplus \text{jn}_{ \Delta,\typeA;\typeE} \comp \sw)  \comp \dist   \comp ([\![v  ]\!] \otimes \text{id}) \comp \text{sp}_{\Gamma; \Delta, \typeA} \comp \text{sh}_{E,\typeA} \comp \text{jn}_{E; \typeA}  & {(\text{coherence})} \\
  & \hspace{10pt} \comp (\text{id} \otimes [\![ t  ]\!] ) \comp \text{sp}_{E;Z} &  \\ 
...
\end{figure}
  ...
  \begin{definition}
    ...
    For two metric spaces $(X, d_X)$ and $(Y, d_Y)$, a morphism $f: (X, d_X) \to (Y, d_Y)$ is a function $f: X \to Y$ satisfying $d_Y(f(x), f(y)) \cdot d_X(x, y)$ for all $x, y \in X$.
    \end{definition}


\begin{definition}
 A category $\catC$ is enriched over metric spaces if for each pair of objects $X$ and $Y$ in $\catC$, the hom-set $\catC(X, Y)$ is a metric space and  if the composition of $\catC$-morphisms,
...
Note that if $\catC$ is a category enriched over metric spaces, then $\catC \otimes \catC$ is also a category enriched over metric spaces.


\begin{definition}
  A given category $\catC$ with coproducts is said to be enriched over metric spaces if in addition to the properties of a category enriched over metric spaces, the following holds: for $f, f'  \in \catC(X,Z)  $ and $g,g'  \in \catC(Y,Z) $, $\catC(X,Z)  \otimes \catC(Y,Z) \rightarrow \catC(X \oplus Y,Z) $ is a functor in the category of metric spaces.
 \end{definition}


  Let $\catC$ be a category with coproducts enriched over metric spaces, it holds that for $f, f'  \in \catC(X,Z)  $ and $g,g'  \in \catC(Y,Z) $, $d_{\catC(X,Z) \otimes \catC(Y,Z)}((f,f'),(g,g')) = \sup(d(f,f'),d(g,g'))$.



\todo[inline]{Present the interpretation of the equational system via monoidal closed categories with coproducts and enriched over metric spaces -> ???}



Consider a symmetric monoidal closed category with coproducts enriched over metric spaces $\catC$. A metric equation $\Gamma \vljud v =_{q} w : \typeA $ is satisfied by this interpretation if $q \leq d( \llbracket \Gamma  \vljud v : \typeA \rrbracket, \llbracket\Gamma \vljud w : \typeA \rrbracket)$ where $d : \catC(\llbracket \Gamma \rrbracket, \llbracket \typeA \rrbracket) \times \catC(\llbracket \Gamma \rrbracket, \llbracket \typeA \rrbracket) \rightarrow \mathbb{R}_0^+$ is the underlying function of the category  enriched over metric spaces $\catC(\llbracket \Gamma \rrbracket, \llbracket \typeA \rrbracket)$.

\begin{theorem}
  The rules in Figures \ref{fig:equations-in-context-cond} and \ref{fig:metric conditionals} are sound for a symmetric monoidal closed category with coproducts enriched over metric spaces $\catC$ over metric spaces. Specifically, if $\Gamma \vljud v =_{q} w : \typeA $ results from the rules in Figures \ref{fig:equations-in-context-cond} and \ref{fig:metric conditionals} then $q \geq d( \llbracket \Gamma  \vljud v : \typeA \rrbracket, \llbracket\Gamma \vljud w : \typeA \rrbracket)$.
\end{theorem}
...
\begin{proof}
 ...
 Concerning the rule in Figure \ref{fig:equations-in-context-cond}, it follows from the fact that $\catC$ is enriched over metric spaces and has coproducts.
  ...
  The second step follows from the fact that $\text{sp}_{\Gamma;\Delta} \comp \text{sh}_{E}$  is a morphism in $\catC$  and that $\catC$ is enriched over metric spaces.  The third and fifth step follow from an analogous reasoning. The fourth step follows from  the premises of the rule in question and from the metric equation ($\otimes_e$). The sixth step follows from the fact that $\catC$ is has coproducts and is enriched over metric spaces. Finally, the last step follows from the premise of the rule in question.
...
It is easy to see that \textbf{Values}$((\typeA,\typeB),d)$ is a (possibly large) category enriched over metric spaces. We then quotient this category enriched over metric spaces into a separated category enriched over metric spaces which we denote by (\textbf{Values}$(\typeA,\typeB),d)$/$\sim$. We call  $\mathscr{T} $\emph{varietal} if (\textbf{Values}$(\typeA,\typeB),d)$/$\sim$ is a small category enriched over metric spaces for all types $\typeA$ and $\typeB$. For the rest of this work, we will focus exclusively on varietal theories and locally small categories.
...
\end{theorem}
\begin{proof}
  We use a strategy similar to the proof of Theorem \ref{theorem:comp_eq_in_context} and take advantage of the quotienting of a category enriched over metric spaces into a separated category enriched over metric spaces. Analogously to Theorem \ref{theorem:comp_eq_in_context} we have a category Syn$(\mathscr{T})$ whose objects are the types of the language and whose hom-sets are the underlying sets of the categories enriched over metric spaces (\textbf{Values}$(\typeA,\typeB),d)$/$\sim$.
  ...
  The next step is to show that if an equation $\Gamma \vljud v =_q v' : \typeA$ with $q \in B$ is satisfied by Syn(T ) then it is a theorem of the linear metric $\lambda-theory$. By assumption $d([v],[v']) = d(v,v') =  \inf{ \{r \, \vert \, v =_r v'\}} \leq q$. It follows from the definition of the way-below relation that for all
