...
The second case follows by induction, the coherence theorem for symmetric monoidal categories, the universal property of the coproduct, and naturality of $\alpha,$ $\lambda,$ $\rho,$ and $\text{sw}$.
...
\begin{tikzpicture}
  ...
  \path[-stealth]
    ...
    %(m-1-1) edge [bend left=5] node [pos=0.5, shift={(0.75, -0.5)}] {$\text{dist}$} (m-2-1)
    (m-1-1)  edge[bend right=5] node [above==0.2cm] {$[\text{id} \otimes i_1,\text{id} \otimes i_2]$} (m-2-1)
