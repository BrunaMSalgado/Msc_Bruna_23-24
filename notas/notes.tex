...
For instance, if the norm considered is the trace norm, let $\psi$ be the trace operator and $\phi_1 = \id $ and $\phi_1' = D_p $, where $D_p$ is the dephasing channel. Then, $\phi_1 - \psi_1'$ maps a density operator 
$$\rho=|\alpha|^{2} |0\rangle \langle 0 | + \alpha \beta^{\dag} |0\rangle \langle 1 | + \alpha^{\dag} \beta |1\rangle \langle 0 | + |\beta|^{2} |1\rangle \langle 1 |$$ 
to 
$$ p \cdot (\alpha \beta^{\dag} |0\rangle \langle 1 | + \alpha^{\dag} \beta |1\rangle \langle 0 |).$$ 
The trace norm of this operator is 
\begin{align*}
...
\todo[inline]{explicar trace norm aqui}
...
where $\tracenorm{-}$ denotes the trace norm. This norm is defined on operators $A =  \begin{pmatrix} A_1 & \ldots & A_n \end{pmatrix}^T \in \mathcal{\sigma} $ and superoperators  $\phi: \sigma \rightarrow \mu$ as follows:
...
Considering $\sigma = n_1, \ldots, n_s$, $\tau = m_1, \ldots, m_t$, $\mu = o_1, \ldots, o_r$, we define $\phi_i= \begin{pmatrix}\phi_{i1}, \ldots, \phi_{ir}\end{pmatrix}^T$ for $i \in 1, \ldots, s$ and  $\psi_i= \begin{pmatrix}\psi_{i1}, \ldots, \psi_{it}\end{pmatrix}^T$ for $i \in 1, \ldots, t$.
...
$\sem{\emph{q} \hspace{1pt}}((a,b)) = \big(\begin{smallmatrix}
  ...
  0 & b
\end{smallmatrix}\big)$, 
for $\psi \in \{0, 1\}$ we define $\sem{\ket{\psi}} (1)= \ket{\psi}$,
