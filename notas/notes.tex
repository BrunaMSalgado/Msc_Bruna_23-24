...
We now consider a slightly modified $\lambda$-theory, distinct from the one presented above: we no longer consider CPTP operations and introduce a new type, $\emph{Iqbit}$, representing qubits in an infinite-dimensional (separable) Hilbert space. Alongside this, we define a predetermined set of operations on $n$ "infinite qubits", such as $U_I\colon \emph{Iqbit}, \ldots, \emph{Iqbit} \to \emph{Iqbit}^{\otimes n}$.
...
The category  $\WstarCPSUop$ is a model of the metric $\lambda$-theory for quantum computation under the following interpretation: are ``imported from'' $\catQ$
$\sem{\typeI}$, 
$\sem{\emph{qbit} \hspace{1pt}}$, $\sem{\emph{q} \hspace{1pt}}$, $\sem{\ket{\psi}}$ for $\psi \in \{0, 1\}$, $\sem{\emph{meas}}$ and $\sem{U}$. To these we add $\sem{\emph{Iqbit} \hspace{1pt}}= \mathcal{B}(\ell^2(\mathbb{Z}))$ and $\sem{U_I}= \rho \mapsto U_I \rho \, U_I^\dag$.
...
\begin{example} [Quantum walk]
  \todo[inline]{Descrição Quantum Walk} %-> p.12 e robustness
\end{example}


 \todo[inline]{Estou aqui}
