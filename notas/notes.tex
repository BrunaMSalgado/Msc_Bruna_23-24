...
\begin{theorem} \cite[Theorem 1.22.6]{sakaiCAlgebrasWAlgebras1998} \label{thm:sakai_tensor_min}
  Let \( \mathscr{A}\) and \( \mathscr{B} \) be two unital \( C^* \).
Then the minimal \( C^* \)-norm \( \|\cdot\|_{\min} \) on \( A \otimes B \) is the least \( C^* \)-norm among all \( C^* \)-norms \( \alpha \) on \( \mathscr{A} \odot \mathscr{B} \). That is,
\[
\|x\|_{\min} \leq \|x\|_{\alpha} \quad \text{for all } x \in A \odot B.
\]
\end{theorem}

\begin{proposition} \cite[Proposition 1.22.7]{sakaiCAlgebrasWAlgebras1998} \label{prop:sakai_tensor_min}
  Let \( \mathscr{A}\ \) and \( \mathscr{B}\ \) be \( C^* \)-algebras, and let \( \alpha \) be a \( C^* \)-norm on the algebraic tensor product \( A \odot B \) such that
\[
...
\begin{proposition} \cite[Exercise 116 III]{westerbaanCategoryNeumannAlgebras2019} \label{prop:norm_tensor_w_mult}
   Let \( \mathscr{A}\ \) and \( \mathscr{B}\ \) be \( W^* \)-algebras, $\norm{a\otimes b}= \norm{a}\norm{b}$ for all $a \in \mathscr{A}$ and  $b \in \mathscr{B}$.
\end{proposition}

\begin{proposition} \cite[Exercise 114 II]{westerbaanCategoryNeumannAlgebras2019} \label{prop:tenso_w_unique}
   The tensor product of von Neumann algebras $\mathscr{A}$ and $\mathscr{B}$ is unique in the sense that when $\gamma \colon \mathscr{A} \times \mathscr{B} \to \mathscr{T}$ and $\gamma' \colon \mathscr{A} \times \mathscr{B} \to \mathscr{T'}$ are tensor products of $\mathscr{A}$ and $\mathscr{B}$, then there is a unique normal miu-isomorphism $\varphi \colon \mathscr{T} \to \mathscr{T'}$ with $\varphi(\gamma(a,b)) = \gamma'(a,b)$ for all $a \in \mathscr{A}$ and $b \in \mathscr{B}$. In other words, the tensor product of von Neumann algebras $\mathscr{A}$ and $\mathscr{B}$ is unique up to unique normal miu-isomorphism.
\end{proposition}

Note that $\varphi$ is an $*$-isomorphism and therefore an isometry.
...
\]
for all \( x, y \in X \).  
Any uniformly continuous function is obviously continuous.  
An important property of uniformly continuous functions is that they map Cauchy sequences into Cauchy sequences \cite{aliprantisBanachLattices1999}. 
\end{definition}

\begin{definition} \label{def:lip_uni_cont}
  A function \( f : (V, d) \to (W, d') \) between metric spaces is said to be \emph{Lipschitz continuous} if there exists a real number \( c \geq 0 \) such that for every \( v_1, v_2 \in V \),
\[
d'(f(v_1), f(v_2)) \leq c \cdot d(v_1, v_2).
\]
The number \( c \) is called a \emph{Lipschitz constant} for \( f \).  
Clearly, every Lipschitz continuous function is uniformly continuous  \cite{aliprantisBanachLattices1999}.
\end{definition}
...
This continuous extension \( g \) exists if \( (V,d) \) is a metric space, \( (W,d') \) is a complete metric space, and \( f \) is uniformly continuous from \( (A,d) \) to \( (W,d') \). 
\end{lemma}

%exercicio do Abrafam 
...
\begin{align*}
  ...
  = & \max \left\{\sup_{\mathcal{D}} \norm{\id_{\mathcal{D}}\, \odot\phi_1} , \sup_{\mathcal{D}}  \norm{\id_{\mathcal{D}}\, \odot\phi_2} \right\}
  ...
  Considering Theorem \ref{thm:beta_alg_eq_beta_gamma}, we know that $\norm{\id_{\mathcal{D}} \odot\phi_1} = \norm{\id_{\mathcal{D}} \overline{\otimes}\phi_1}$. Consequently, the next step is to prove that $ \norm{\id_{\mathcal{D}}\, \widecheck{\otimes}\, \phi} \leq \norm{\id_{\mathcal{D}} \, \overline{\otimes} \, \phi}$. Given Propositions \ref{prop:sakai_tensor_min} and \ref{prop:norm_tensor_w_mult}, we have that $$\norm{v}_\text{min} \leq \norm{v}  \quad \text{for all } v \in \mathscr{A} \widecheck{\otimes} \mathscr{B}.$$
  
  This means that in order to prove that the proposition holds it suffices to prove that for all $ v \in \mathscr{A} \widecheck{\otimes} \mathscr{B}$, $\phi \, \widecheck{\otimes} \, \psi (v) = \phi \, \overline{\otimes} \, \psi (v)$.


  This follows because the three diagrams below commute, which is a consequence of the universal property of the algebraic tensor product, Theorem \ref{thm:beta_alg_eq_beta_gamma}, and Proposition Lemma~\ref{lem:completion_unique}, together with the observation that $\id \odot \phi$ is subunital and hence Lipschitz continuous. 
...
V \times W \arrow[r, "T"] \arrow[d, "\odot",shorten >=2pt, shorten <=2pt] &  Z\\
{V \odot W}\arrow[d, hookrightarrow, "\iota",right, shorten >=2pt, shorten <=2pt]  \arrow[ur,  dashed, "T'",shorten >=2pt, shorten <=2pt]  \\
V \widehat{\otimes}W \arrow[uur,dashed,"\widehat{T}",shorten >=2pt, shorten <=2pt]\\
\end{tikzcd}
\end{minipage}
 \hspace{-110pt}
\begin{minipage}{0.45\textwidth}
...
\begin{tikzcd}[row sep=4em, column sep=6em]
V \times W \arrow[r, "T"] \arrow[d, "\odot",shorten >=2pt, shorten <=2pt] &  Z\\
{V \odot W}\arrow[d, hookrightarrow, "\iota",right, shorten >=2pt, shorten <=2pt]   \\
V \overline{\otimes}W \arrow[uur,dashed,"\widehat{T}",shorten >=2pt, shorten <=2pt]\\
\end{tikzcd}
 \hspace{-100pt}
\end{minipage}
\begin{minipage}{0.45\textwidth}
...
\begin{tikzcd}[row sep=4em, column sep=6em]
 Z\\
{V \odot W}\arrow[d, hookrightarrow, "\iota_1",right, shorten >=2pt, shorten <=2pt]  \arrow[dr,  hookrightarrow, "\iota_2",shorten >=2pt, shorten <=2pt]  \arrow[u, dashed, "T'", shorten >=2pt, shorten <=2pt]     \\
V \widehat{\otimes}W \arrow[r,  hookrightarrow, "\iota_3"] \arrow[uu,bend left=50, dashed, "\widehat{T}", shorten >=2pt, shorten <=2pt ]  & V \overline{\otimes}W \arrow[uul,  dashed, "\overline{T}", shorten >=2pt, shorten <=2pt]  \\
\end{tikzcd}
\end{minipage}
\]
In the third diagram, we reason as follows:
from the first diagram above, we obtain $ \widehat{T} \cdot \iota_1 = T =  \overline{T} \cdot \iota_2$, then, given that $\iota_2 = \iota_3 \cdot \iota_1$, we have  $ \widehat{T} \cdot \iota_1 =  \overline{T} \cdot \iota_3 \cdot \iota_1$, from where if follows that $\overline{T} \circ \iota_3 = \widehat{T}$.
...
\begin{proposition}
  The definitions of completely positivity presented are equivalente for maps 
  $$\Phi: \mathcal{M}_{n1} \oplus \ldots \oplus  \mathcal{M}_{ns} \to \mathcal{M}_{m1} \oplus \ldots \oplus  \mathcal{M}_{mt}. $$
\end{proposition}

\begin{proof}
  We begin by observing that the composition of positive maps is positive, and that the map which swaps tensor factors is itself positive \cite{watrous2018theory}. Therefore, Definition~1 is equivalent to the one obtained by replacing \( \Phi \otimes \id_{\mathcal{T}(\HilbH)} \) with \( \id_{\mathcal{T}(\HilbH)} \otimes \Phi \). 

  We now proceed to demonstrate the equivalence between this definition and Definition 2. Here we note the isometric isomorphisms $\mathcal{B}(\HilbH)\cong \SqMatrix{n}$ \cite{watrous2018theory} with $\text{dim}(\HilbH)=n$ and $\mathcal{M}_o(\mathcal{M}_{n1} \oplus \ldots \oplus  \mathcal{M}_{ns} )\cong \mathcal{M}_o \otimes (\mathcal{M}_{n1} \oplus \ldots \oplus  \mathcal{M}_{nt})$ \cite[Corollary 8.1.3]{effrosOperatorSpaces2000}. It is straightforward that the first isomorphism is positive. Now, regarding the second isomorphism, consider that $[a_{i,j}] \in \mathcal{M}_o(\mathcal{M}_{n1} \oplus \ldots \oplus  \mathcal{M}_{nr})$ is positive, \ie $[a_{i,j}] = [b_{i,j}]^* [b_{i,j}] $. Note that
  \begin{align*}
...
The isomorphism $i:\mathcal{M}_o(\mathcal{M}_{n1} \oplus \ldots \oplus  \mathcal{M}_{nr} ) \to \mathcal{M}_o \otimes (\mathcal{M}_{n1} \oplus \ldots \oplus  \mathcal{M}_{nr})$ is defined as $i([a_{i,j}]) = \sum_{i,j}  \ket{i}\bra{j} \otimes a_{ij} $,
...
Let $\sigma:1, \ldots, s$, and $\tau:1, \ldots, t$, the equivalence of definitions 1 and 2, follows from the fact that the two diagrams below commute and the composition of positive maps is positive. For the first we assume that $\id_{\mathcal{T}(\HilbH)} \otimes \Phi$ is positive for all finite dimensional $\mathcal{H}$, and for the second that $\SqMatrix{o}(\Phi)$ is positive for all $o \in \mathbb{N}$. 
...
\begin{tikzpicture}
  ...
  {
  \mathcal{T}(\HilbH) \otimes  \SqMatrix{\sigma}&  \mathcal{T}(\HilbH) \otimes  \SqMatrix{\tau}  \\
  \SqMatrix{o} \otimes  \SqMatrix{\sigma} &  \SqMatrix{o} \otimes  \SqMatrix{\tau}  \\
  \SqMatrix{o}( \SqMatrix{\sigma}) &  \SqMatrix{o}( \SqMatrix{\tau}) \\
  };
  \path[-stealth]
    (m-1-1) edge  node [above] {$\id_{\mathcal{T}(\HilbH)} \otimes \Phi  $} (m-1-2)
    (m-1-2) edge   node [right] {$\cong $} (m-2-2)
    (m-2-2) edge  node [right] {$ A \otimes B \mapsto [(a_{i,j} B)_{i,j}] $} (m-3-2)
    (m-2-1) edge  node [right] {$ \cong $} (m-1-1)
    (m-3-1) edge  node [right] {$[a_{i,j}] \mapsto \sum_{i,j}  \ket{i}\bra{j} \otimes a_{ij} $} (m-2-1)
    (m-3-1) edge  node [below] {$\SqMatrix{o}(\Phi)$} (m-3-2)
    ;
\end{tikzpicture}
\end{minipage}
\hspace{20pt}
...
\begin{tikzpicture}
  ...
  \path[-stealth]
    ...
    (m-3-2) edge  node [right] {$ [a_{i,j}] \mapsto \sum_{i,j}  \ket{i}\bra{j} \otimes a_{ij}$} (m-2-2)
    (m-1-1) edge  node [right] {$ \cong $} (m-2-1)
    (m-2-1) edge  node [right] {$ A \otimes B \mapsto [(a_{i,j} B)_{i,j}] $} (m-3-1)
    (m-3-1) edge  node [below] {$\SqMatrix{o}(\Phi)$} (m-3-2)
    ;
\end{tikzpicture}
\end{minipage}
...
First, we note that the definition of unitary operators extends for separable Hilbert spaces \cite{heinosaariMathematicalLanguageQuantum2011}, and from that it follows that they are unital.  Their complete positivity is established by the following theorem.

%\todo[inline]{Dúvida para professor Renato: De uma maneira geral há $3$ formas de definir completa positividade para um superop $f$: a das C$^*$-Algebras com M$_n(f)$ a ter de ser pos para cada $n \in \mathbb{N}$, e depois nos espaços de Hilbert diferentes autores definem como $\id_{B(H)} \otimes f$ ou $f \otimes \id_{B(H)}$ deve ser pos para qualquer $H$. Por exemplo o Selinger e o Chuang usam a primeira e é facil ver a correspondencia desta para a das C$^*$ algebras. No entanto, por exemplo, o Watrous usa a segunda. A minha questão é o $f \otimes \id_{B(H)}$ também é equivalente às restantes? Porque eu não estou a conseguir ver como.}

\begin{theorem} \cite[Theorem 1.6]{pisierIntroductionOperatorSpace2003}
Let \( \mathcal{H}_1 \), \( \mathcal{H}_2 \) and \( \mathcal{K} \)  be Hilbert spaces, let \( \pi :  \mathcal{K} \to \mathcal{B}(\mathcal{H}_2) \) be a $*$-homomorphism, and let \( V_1 : \mathcal{H}_1 \to \mathcal{H}_2 \), and \( V_2 : \mathcal{H}_2 \to \mathcal{H}_1 \) be bounded operators. Define a map \( \Phi :  \mathcal{B}(\mathcal{K})  \to \mathcal{B}(\mathcal{H}_1) \) by
\[
\Phi(a) = V_2 \pi(a) V_1.
\]
Then \( \Phi \) is completely bounded with
\[
\| \Phi \|_{\mathrm{cb}} \leq \|V_1\| \cdot \|V_2\|,
\]
and if \( V_1 = V_2^{\dag} \), then \( \Phi \) is completely positive.
\end{theorem}

The next propositions will be useful to compute norms in the example that follows.

\begin{proposition} \cite[Proposition 8.11] {paulsenCompletelyBoundedMaps2003} \label{prop:Mn_cb_norm}
  Let \( \Phi : \mathcal{M}_{m} \to \mathcal{M}_n \) be a linear map. Then
\[
\|\varphi_n\| = \|\varphi\|_{\text{cb}}.
\]
\end{proposition}

\begin{proposition}  \cite[Exercise 3.10]{paulsenCompletelyBoundedMaps2003}  \label{prop:Mn_phi_n_norm}
  Let \( \mathscr{A} \) and \( \mathscr{B} \) be unital \( C^* \)-algebras, and let \( \Phi: \mathscr{A} \to \mathscr{B} \) be a bounded linear map.  Then,
\[
\|\Phi_n\| \leq n \|\Phi\|.
\]
\end{proposition}
...
$\sem{\typeI}$, 
$\sem{\emph{qbit} \hspace{1pt}}$, $\sem{\ket{\psi}}$ for $\psi \in \{0, 1\}$, and $\sem{U}$ are ``imported from'' $\catQ$. To these we add $\sem{\emph{pos} \hspace{1pt}}= \mathcal{B}(\ell^2(\mathbb{Z}))$ and $\sem{S}= \rho \mapsto S \rho \, S^\dag$.
...
First, we compute:
\begin{align*}
\id (\rho) - \mathcal{E}(\rho) &=    |\alpha|^{2} \ket{0} \bra{0} + \alpha \beta^{*} \ket{0} \bra{1} + \alpha^{*} \beta \ket{1} \bra{0} + |\beta|^{2} \ket{1} \bra{1}  - \big( p(I/ 2) + (1-p)   \\ 
  & \hspace{10pt}  \big(|\alpha|^{2} \ket{0} \bra{0} +  \alpha \beta^{*} \ket{0}\bra{1} +  \alpha^{*} \beta \ket{1}\bra{0} + |\beta|^{2} \ket{1}\bra{1}\big)\big)\\
& = p \left( (|\alpha|^2-1/2) \ket{0}\bra{0} +  \alpha \beta^{*} \ket{0} \bra{1} + \alpha^{*} \beta \ket{1} \bra{0} + (|\beta|^{2} -1/2) \ket{1} \bra{1} \right)
\end{align*} 
Considering that give a quantum state $\rho$ its cartesian coordinates are given by 
\begin{equation*}
...
As a result, given that for $A \in \TraceOp{\HilbH}$, it holds that $\opnorm{A} \leq  \tracenorm{A}$ \cite{heinosaariMathematicalLanguageQuantum2011,watrous2018theory},
it follows:
\begin{align*}
  \opnorm{\id (\rho) - \mathcal{E}(\rho)} \leq \tracenorm{\id (\rho) - \mathcal{E}(\rho)} &= \euclideannorm{(p(\alpha \beta^{*} + \alpha^{*} \beta), p \cdot i(\alpha \beta^{*} - \alpha^{*} \beta) , p( |\alpha|^2-|\beta|^2))} \\
...
\todo[inline]{NOTA:Alternativamente se a Proposição 3.46 estiver correta, como temos $\norm{\SqMatrix{o}(\Phi)} = \norm{\id_{\mathcal{T}(\HilbH)} \otimes \Phi}$, podemos ir pela diamond norm que dá um bound mais pequeno.}


Attending to Propositions \ref{prop:Mn_cb_norm} and \ref{prop:Mn_phi_n_norm}, it holds that
\begin{align*}
  \cbnorm{ \id - \mathcal{E}} \leq 2p
\end{align*}
...
\begin{align*}
  &\textbf{StepQWalk}=_{2p} \textbf{StepQWalk}
\end{align*}
