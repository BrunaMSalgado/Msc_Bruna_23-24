...
\begin{definition}
  A given a category $\catC$ with coproducts it is said to be enriched over metric spaces if in addition to the properties of a category enriched over metric spaces, the following holds: for $f, f'  \in \catC(X,Z)  $ and $g,g'  \in \catC(Y,Z) $, $\catC(X,Z)  \otimes \catC(Y,Z) \rightarrow \catC(X \oplus Y,Z) $ is a funtor in the category of metric spaces.
 \end{definition}
...
\begin{proof}
  ...
  In relation to the rule in Figure \ref{fig:equations-in-context-cond} it follows from the fact that $\catC$ is enriched over metric spaces and has coproducts.
