...
\begin{proof}
  ...
  \begin{align*}
    \big[z:(\typeA \oplus \mathbb{\typeB}) \otimes \mathbb{\typeD} \vljud \text{pm } z \text{ to } a \otimes b. \text{ case } a\,\{ &\inl_{\typeB}(x) \Rightarrow \inl_{\typeB \otimes \typeD}(x \otimes b); \\
...
\end{proof} 
  ...
  where each \(p_i\) is the probability assigned to \(i \in N\), and \(\delta_i\) denotes the Dirac measure at \(i\).
    ...
    \begin{tikzpicture}
      \matrix (m) [matrix of math nodes, row sep=4em, column sep=6em, minimum width=2em]
      {
        \mathcal{M}\mathbb{R}  & \mathbb{R} \,   \widecheck{\otimes}_{\pi} \mathcal{M}\mathbb{R} \,  \, & \mathcal{M}\mathbb{R} \, \widecheck{\otimes}_{\pi} \,  \mathcal{M}\mathbb{R} &  \mathcal{M} \left( \mathbb{R} \times \mathbb{R} \right)  \\
        \mathcal{M}\mathbb{R}  \\
      };
      \path[-stealth]
        (m-1-1) edge node [left] {$\sem{\emph{JumpL}_\mu}$} (m-2-1)
        (m-1-1) edge node [above] {$\cong$} (m-1-2)
        (m-1-2) edge node [above] {$f\,  \widecheck{\otimes}_{\pi} \,  \id $} (m-1-3)
...
\begin{align*}
  ...
  This follows because the three diagrams below commute, which is a consequence of the universal property of the algebraic tensor product, Theorem \ref{thm:beta_alg_eq_beta_gamma}, and  Lemma~\ref{lem:completion_unique}, together with the observation that $\id \odot \phi$ is subunital and hence Lipschitz continuous. 
...
V \widecheck{\otimes}W \arrow[uur,dashed,"\widecheck{T}",shorten >=2pt, shorten <=2pt]\\
\end{tikzcd}
\end{minipage}
 \hspace{-110pt}
\begin{minipage}{0.45\textwidth}
...
V \overline{\otimes}W \arrow[uur,dashed,"\widecheck{T}",shorten >=2pt, shorten <=2pt]\\
\end{tikzcd}
 \hspace{-100pt}
\end{minipage}
\begin{minipage}{0.45\textwidth}
...
V \widecheck{\otimes}W \arrow[r,  hookrightarrow, "\iota_3"] \arrow[uu,bend left=50, dashed, "\widecheck{T}", shorten >=2pt, shorten <=2pt ]  & V \overline{\otimes}W \arrow[uul,  dashed, "\overline{T}", shorten >=2pt, shorten <=2pt]  \\
\end{tikzcd}
\end{minipage}
\]
...
\begin{proof}
  Firstly, we establish that $\WstarCPSUop$ is $\catMet$-enriched. By unpacking the relevant definitions, this reduces to proving the following: for all short operators $\phi, \phi' : \mathcal{A} \to \mathcal{B}$ and $\psi, \psi' : \mathcal{B} \to \mathcal{C}$ the inequation $\cbnorm{\phi-\phi'} + \cbnorm{\psi-\psi'} \geq \cbnorm{\psi\phi-\psi'\phi'} $ holds. Considering \cite[ Proof of proposition 4.1]{dahlqvist2023syntactic},  it is sufficient to establish that $\cbnorm{\psi(\phi-\phi')} \leq \cbnorm{\phi-\phi'}$ and $\cbnorm{(\psi-\psi')\phi} \leq \cbnorm{\psi-\psi'}$ which follows the fact that the cb-norm is obviously submultiplicative with respect to composition, from Proposition \ref{prop:cp_cb} and maps $\phi$ and $\psi$  completely positive subunital, and therefore, short maps.
...
where $\HilbH, \HilbK$ are finite-dimensional.
\end{proposition}

\begin{proof}
  We begin by observing that the composition of positive maps is positive, and that the map which swaps tensor factors is itself positive \cite{watrous2018theory}. Therefore, Definition~1 is equivalent to the one obtained by replacing \( \Phi \otimes \id_{\mathcal{B}(\HilbH)} \) with \( \id_{\mathcal{B}(\HilbH)} \otimes \Phi \). 

  We now proceed to demonstrate the equivalence between this definition and Definition 2. Here we note the isometric isomorphisms $\mathcal{M}_n \otimes \mathcal{M}_m \cong \mathcal{M}_{nm}$, $\mathcal{B}(\HilbH)\cong \SqMatrix{n}$ \cite{watrous2018theory} with $\text{dim}(\HilbH)=n$ and $\mathcal{M}_n(\BoundOp{\HilbH})\cong \mathcal{M}_n \otimes \BoundOp{\HilbH}$ \cite[Corollary 8.1.3]{effrosOperatorSpaces2000}. It is straightforward that the first two isomorphisms are positive. Now, regarding the third isomorphism, consider that $[a_{i,j}] \in \mathcal{M}_n(\BoundOp{\HilbH})$ is positive, \ie $[a_{i,j}] = [b_{i,j}]^* [b_{i,j}] $. Note that
  \begin{align*}
...
The equivalence of definitions 1 and 2, follows from the fact that the two diagrams below commute and the composition of positive maps is positive. For the first we assume that $\id_{\mathcal{T}(\HilbH)} \otimes \Phi$ is positive for all finite dimensional $\mathcal{H}$, and for the second that $\SqMatrix{n}(\Phi)$ is positive for all $m \in \mathbb{N}$. 
...
\todo[inline]{NOTA:Alternativamente se a Proposição 3.46 estiver correta, como temos $\norm{\SqMatrix{n}(\Phi)} = \norm{\id_{\mathcal{T}(\HilbH)} \otimes \Phi}$, e portanto temos uma metrica que automaticamente respeita as condições. Assim, a primeira frase do texto que se segue saía.}
