...
As noted in \Cref{subsec:interlude_bool}, a metric equational system for the conditionals would be extremely helpful for reasoning about approximate equivalence in the setting of programming and beyond.
In this chapter, we address this gap by introducing such a system and corresponding models, and we then establish a soundness and completeness result in the same style as before. Additionally, we illustrate our system at work via two small examples: reasoning about approximate equivalence between boolean terms (i.e., terms of type $\typeI \oplus \typeI$) and a metric version of copairing's extensionality. 
...
%Actually, or equational system encompasses both \autoref{fig:metric conditionals} and \autoref{fig:metric deductive system}
% We note that the \inl and \inr equations are redundant, they can be deduced/derived from the substitution rule in \autoref{fig:metric deductive system}
%Observe that whilst in the original quantalic system the operation ``+'' assumes a preponderant,  in the extended version $\sup$ is given a similar role.
%Intuitively this equation gives a bound for the worst case scenario: in which either $w$ or $u$ are executed --- there's no in between (by this we mean that under certain interpretations for instance a probabilistic ``mixture'' of programs may happen)--- and therefore we take the biggest error as the bound.
...
\begin{align*}
  ...
  Thus, we obtain a category $\catSyn(T)$ whose objects are the types of the language and whose hom-sets are the underlying sets of the  $\catMet$-category (\textbf{Values}$(\typeA,\typeB),d)$/$\sim$.


  The next step is to show that $\catSyn(T)$ is a symmetric monoidal $\catMet$-category with binary coproducts. To this effect, with repect to the case statement, we reason as follows:
  ...
  The final step is to show that if an equation $\Gamma \vljud v =_q v' : \typeA$ with $q \in [0, \infty]$ is satisfied by Syn$(T)$ then it is a theorem of the linear metric $\lambda$-theory. By assumption, $d([v],[v']) = d(v,v') =  \inf{ \{r \, \vert \, v =_r v'\}} \leq q$. It follows from the definition of the way-below relation that for all
