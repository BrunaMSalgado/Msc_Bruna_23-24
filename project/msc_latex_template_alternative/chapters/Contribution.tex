...
\begin{lemma}\label{thm:Russo–Dye}
  For all super-operators $Q_i: \mathbb{C}^{o_i \times o_i} \rightarrow \mathbb{C}^{p_1 \times p_1} \oplus \ldots \oplus \mathbb{C}^{p_m \times p_m}$, it holds that: 
  \begin{equation}
    \lVert Q_i \rVert_{1 \text{ gen}} = \max \{ \text{Tr}\left(Q_i(uu^{*}) \right) \mid \lVert u \rVert_{1}=1 \}
  \end{equation}
\end{lemma}
