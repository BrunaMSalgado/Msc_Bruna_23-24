...
Within this model, the operators take the form $Q: V_1 \oplus \ldots \oplus V_n  \rightarrow  W_1 \oplus \ldots \oplus W_m$, where each $V \in \{V_1, \ldots, V_n, W_1, \ldots, W_n\}$ is a vector space without direct sums.
...
\begin{definition} \label{def:gen_norm_gen_inj}
  For $Q: V \rightarrow W_1 \oplus \ldots \oplus W_m$, (where  $W_1, \ldots, W_m$  are not direct sum of vector spaces,) $\exists$ $Q_1: V \rightarrow W_1, \ldots,Q_2: V \rightarrow W_m$ such that $$Q = Q_1 + \ldots +  Q_m,$$ where $Q_1 (v) = \underbrace{\textsc{Il} \cdot \ldots \cdot \textsc{Il}}_{m-1 \times } \cdot \underbrace{\textsc{Pl} \cdot \ldots \cdot \textsc{Pl}}_{m-1 \times } (Q (v)), \ldots, Q_M (v) = \underbrace{\textsc{Ir} \cdot \ldots \cdot \textsc{Ir}}_{m-1 \times } \cdot  \underbrace{\textsc{Pr} \cdot \ldots \cdot \textsc{Pr}}_{m-1 \times } (Q(v))$. The norm of the operator $Q$ is defined as follows:
  \begin{equation}
...
\begin{definition} \label{def:gen_norm_ops}
  ...
  $$Q= [Q_{11} + \ldots +  Q_{1m},  \hspace{2pt} \ldots  \hspace{2pt}, Q_{n1} + \ldots +  Q_{nm}],$$  
  where
  \begin{equation*}
    \begin{split}
      &Q_{11} (v_1) = \underbrace{\textsc{Il} \cdot \ldots \cdot \textsc{Il}}_{m-1 \times} \cdot \underbrace{\textsc{Pl} \cdot \ldots \cdot \textsc{Pl}}_{m-1 \times }  \cdot \hspace{1pt} Q  (\underbrace{\textsc{Il} \cdot \ldots \cdot \textsc{Il}}_{n-1 \times} (v_1)),  \hspace{1pt} \ldots \hspace{1pt} , \\
      &Q_{1m} (v_1) = \underbrace{\textsc{Ir} \cdot \ldots \cdot \textsc{Ir}}_{m-1 \times}\cdot \underbrace{\textsc{Pr} \cdot \ldots \cdot \textsc{Pr}}_{m-1 \times } \cdot \hspace{1pt} Q ( \underbrace{\textsc{Il} \cdot \ldots \cdot \textsc{Il}}_{n-1 \times} (v_1)), \hspace{1pt} \ldots \hspace{1pt},\\ 
      & Q_{n1} (v_n) = \underbrace{\textsc{Il} \cdot \ldots \cdot \textsc{Il}}_{m-1 \times} \cdot \underbrace{\textsc{Pl} \cdot \ldots \cdot \textsc{Pl}}_{m-1 \times } \cdot \hspace{1pt} Q (\underbrace{\textsc{Ir} \cdot \ldots \cdot \textsc{Ir}}_{n-1 \times}(v_n)), \hspace{1pt} \ldots \hspace{1pt},\\
      & Q_{nm} (v_n) = \underbrace{\textsc{Ir} \cdot \ldots \cdot \textsc{Ir}}_{m-1 \times}\cdot \underbrace{\textsc{Pr} \cdot \ldots \cdot \textsc{Pr}}_{m-1 \times } \cdot \hspace{1pt} Q (\underbrace{\textsc{Ir} \cdot \ldots \cdot \textsc{Ir}}_{n-1 \times}(v_n))
    \end{split}
  \end{equation*}
...
\begin{convention}
  From this point forward, unless stated otherwise, whenever the equality $Q = [Q_{11} + \ldots + Q_{1m}, \ldots, Q_{n1} + \ldots + Q_{nm}]$ is used for an operator $ [Q_{11} + \ldots +  Q_{1m},  \hspace{2pt} \ldots  \hspace{2pt}, Q_{n1} + \ldots +  Q_{nm}]$, it is understood that $Q_{11}, \ldots, Q_{1m}, \ldots, Q_{n1}, \ldots, Q_{nm}$ are defined as in \autoref{def:gen_norm_ops}.
\end{convention}
...
\begin{equation}
  \begin{split}
    ...
    & Q \otimes S = [Q_{11} + \ldots + Q_{1m}, \ldots, Q_{n1} + \ldots + Q_{nm}] \otimes [S_{11} + \ldots + S_{1s}, \ldots, S_{t1} + \ldots + S_{ts}]. \\
  \end{split}
\end{equation}
...
\begin{equation}
  \begin{split}
    Q \otimes S : & V_1 \otimes R_1 \oplus \ldots \oplus V_1 \otimes R_t \oplus \ldots \oplus V_n \otimes R_1 \oplus \ldots \oplus V_n \otimes R_t \rightarrow W_1 \otimes Y_1 \oplus \ldots \oplus   \\
     &W_1 \otimes Y_s \oplus \ldots \oplus W_m \otimes Y_1 \oplus \ldots \oplus W_m \otimes Y_s \\
       Q \otimes S = & [Q_{11} \otimes S_{11} + \ldots + Q_{11} \otimes S_{1s}  + \ldots + Q_{1m} \otimes S_{11}+ \ldots + Q_{1m} \otimes S_{1s}, \hspace{2pt} \ldots \hspace{2pt},\\
       &Q_{11} \otimes S_{r1} + \ldots + Q_{11} \otimes S_{rs}  + \ldots + Q_{1m} \otimes S_{r1}+ \ldots + Q_{1m} \otimes S_{rs}, \hspace{2pt} \ldots \hspace{2pt},\\
       &Q_{n1} \otimes S_{11} + \ldots + Q_{n1} \otimes S_{1s}  + \ldots + Q_{nm} \otimes S_{11}+ \ldots + Q_{nm} \otimes S_{1s}, \hspace{2pt} \ldots \hspace{2pt}, \\
       & Q_{n1} \otimes S_{r1} + \ldots + Q_{n1} \otimes S_{rs}  +  \ldots + Q_{nm} \otimes S_{r1}+ \ldots + Q_{nm} \otimes S_{rs}].
  \end{split}
\end{equation}

Consider the identity operator, $I: V_1 \oplus \ldots \oplus V_n \rightarrow  V_1 \oplus \ldots \oplus V_n$. Given that in the decomposition $I= [I_{11} + \ldots +  I_{1n},  \hspace{2pt} \ldots  \hspace{2pt}, I_{n1} + \ldots +  I_{nm}]$ $I_{ij}$ corresponds to the null operator if $i \neq j$ and the identity operator if $i = j$, if follows that $I = [I_{11}, \ldots, I_{nn}] = [I_{V_1}, \ldots, I_{V_n}]$. As a result, given a superoperator $Q: V_1 \oplus \ldots \oplus V_n \rightarrow W_1 \oplus \ldots \oplus W_m$, the superoperator $Q \otimes I$ is defined as follows:
\begin{equation} \label{eq:gen_tensor_identity}
  \begin{split}
  Q \otimes I_{R_1 \oplus \ldots \oplus R_t} = & [Q_{11} \otimes I_{R_1} + \ldots + Q_{1m} \otimes I_{R_1}, \hspace{2pt} \ldots \hspace{2pt}, Q_{11} \otimes I_{R_t} + \ldots + Q_{1m} \otimes I_{R_t}, \hspace{2pt} \ldots \hspace{2pt},\\
  & Q_{n1} \otimes I_{R_1} + \ldots + Q_{nm} \otimes I_{R_1}, \hspace{2pt} \ldots \hspace{2pt}, Q_{n1} \otimes I_{R_t} + \ldots + Q_{nm} \otimes I_{R_t}].
  \end{split}
\end{equation}
