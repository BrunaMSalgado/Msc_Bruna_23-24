...
\begin{proof}
  ...
  \begin{align*}
   \hspace{-30pt} \|Q + S\|_{\{V_1\rightarrow W, \ldots, V_n\rightarrow W\} \text{ gen}} & = \|[Q_1 + S_1, \ldots, Q_n + S_n]\|_{\{V_1, \ldots, V_n\} \text{ gen}} \\
    ...
    & = \max \{\|Q_1\|_{V_1 \rightarrow W}, \ldots, \|Q_n\|_{V_1 \rightarrow W}  \} + \max \{\|S_1\|_{V_1 \rightarrow W}, \ldots, \|S_n\|_{V_1 \rightarrow W}  \} \\
    & = \|Q\|_{\{V_1\rightarrow W, \ldots, V_n\rightarrow W\} \text{ gen}} + \|S\|_{\{V_1\rightarrow W, \ldots, V_n\rightarrow W\} \text{ gen}}
  \end{align*}
\end {proof}


Using \autoref{def:gen_norm}, it follows that the norms of the left and right injections, $\textsc{Ir}: V \rightarrow V \oplus W$ and $\textsc{Il}:V \rightarrow W \oplus V$, applied to a vector $v \in V$, are defined as follows:
  \begin{equation}
    \begin{split}
...
\begin{definition}
  For $Q: V \rightarrow W_1 \oplus W_2$, where $V$, $W_1$ and $W_2$ are not direct sum of vector spaces, $\exists$ $Q_1: V \rightarrow W_1$ and $Q_2: V \rightarrow W_2$ such that $Q = \textsc{Il}(Q_1) + \textsc{Ir}(Q_2)$, where $Q_1 (v) = \textsc{Pl}(Q(v))$ and $Q_2 (v) = \textsc{Pr}(Q(v))$. The norm of the operator $Q$ is defined as follows:
  \begin{equation}
    \|Q\|_{\{V \rightarrow W_1, V \rightarrow W_2\} \text{ gen}} =  \|\textsc{Ir}(Q_1) + \textsc{Il}(Q_2)\|_{\{V \rightarrow W_1, V \rightarrow W_2\}} = \| Q_1\|_{V \rightarrow W_1} + \|Q_2\|_{V \rightarrow W_2}
  \end{equation}
\end{definition}

\begin{definition}
 Particularly, when  $V_1$ and $V_2$ are complex spaces of squares matrices $W$ is a vector space within the model considered in this work and, the norm of the operator $Q: V_1 \otimes V_2 \rightarrow W$, where $V_1$ and $V_2$, is defined as follows:
\begin{equation}
  \begin{cases}
    \|Q\|_{1 gen} = \max \{\|Q_1\|_{1},  \|Q_2\|_{1}  \} & \text{if $W$ is not a direct sum of vector spaces} \\ \\
    \|Q\|_{1 gen}  = \max \{\|Q_1\|_{1 gen},  \|Q_2\|_{1 gen}  \} & \text{if $W$ is a direct sum of vector spaces $W_1$} \\ 
     = \max \{ \| Q_{11}\|_{1} + \|Q_{12}\|_{1},\| Q_{21}\|_{1} + \|Q_{22}\|_{1} \}  &  \text{and $W_2$, which do not have direct sums}
    \end {cases}
\end{equation}
where $Q_1: V_1 \rightarrow W$ is defined as $Q_1(v_1)= \textsc{Il}(Q(v_1))$, $Q_2: V_2 \rightarrow W$ is defined as $Q_1(v_1)= \textsc{Ir}(Q(v_2))$, $Q_{11}: V_1 \rightarrow W_1$, is defined as $Q_{11}(v_1) = \textsc{Pl}(Q_1(v_1))$, $Q_{12}: V_1 \rightarrow W_2$, is defined as $Q_{12}(v_1) = \textsc{Pr}(Q_1(v_1))$, $Q_{21}: V_2 \rightarrow W_1$, is defined as $Q_{21}(v_2) = \textsc{Pl}(Q_2(v_2))$, and $Q_{22}: V_2 \rightarrow W_2$, is defined as $Q_{22}(v_2) = \textsc{Pr}(Q_2(v_2))$.
\end{definition}
%m
