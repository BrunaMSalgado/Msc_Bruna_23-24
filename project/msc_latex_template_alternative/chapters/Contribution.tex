...
Afterward, it is necessary to prove that the identity operator $I$ has a norm equal to 1.
...
\begin{lemma} \label{lem3}
  $  \lVert [\textsc{Il} \otimes I, \textsc{Ir} \otimes I ]  \rVert_{\sigma} = 1 $
\end{lemma}
...
\begin{equation} 
  \begin{split}
    \lVert [\textsc{Il} \otimes I, \textsc{Ir} \otimes I]  \rVert_{\sigma}  \\
  \end{split}   
  \end{equation}

Finally, it is possible to demontrate that $i^{-1}$ has a norm equal to 1

\begin{lemma} \label{lem4}
  $  \lVert i^{-1}  \rVert_{\sigma} = 1 $
\end{lemma}
...
\vspace{10pt}
\textit {Proof.} 
Here, $\text{d}(A,B)$ denotes the distance between super-operators $A$ and $B$, which in the quantum paradigm corresponds to the diamond norm between the two super-operators. Hence, denoting $ [\textsc{Il} \otimes I, \textsc{Ir} \otimes I ]$ by $i$ it follows that:
...
\begin{equation} \label{eq:proof_theorem1.1_esq}
  \begin{split}
    ...
    &=  \lVert [(P - P') \otimes I, (Q-Q') \otimes I]  \rVert_{\sigma} \hspace{235pt} \text{ \{\autoref{lem4}\}} \\
  \end{split}
  \end{equation}
and
...
\begin{equation}
  \begin{split}
    ...
    &=    \lVert  [P-P',Q-Q'] \rVert_{\sigma} \lVert I \rVert_{\sigma}\hspace{2pt} & \hspace {40pt} \text{\{Norm multiplicative with respect to tensor products\}} \\ 
...
\begin{equation} 
  \begin{split}
     ...
     = &  \text{max}\{ \lVert P \otimes I - P' \otimes I \rVert_{\sigma}, \lVert Q \otimes I - Q'\otimes I \rVert_{\sigma} \}\\
     = &  \text{max}\{ \lVert (P - P') \otimes I \rVert_{\sigma}, \lVert (Q - Q') \otimes I \rVert_{\sigma} \}\\
     = &\text{max}\{ \lVert (P - P') \rVert_{\sigma} \lVert  I \rVert_{\sigma}, \lVert (Q - Q') \rVert_{\sigma} \lVert I \rVert_{\sigma} \} &  \text{\{Norm multiplicative with respect to tensor products\}} \\
