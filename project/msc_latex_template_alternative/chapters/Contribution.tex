...
\todo[inline,size=\normalsize]{Por tudo relativo a operacoes direito:  definir operacoes nao unitarias}
...
we want to prove that te($E$) $\simeq_{\pi}$  te($E'$). By induction hypothesis, te($\Gamma$) $\simeq_{\pi}$  te($\Gamma'$), te($\Delta,x$) $\simeq_{\pi}$  te($\Delta',x'$) and te($\Delta,y$) $\simeq_{\pi}$  te($\Delta',y'$). Given that both case statements are well-typed, this implies that  te($\Delta$) $\simeq_{\pi}$  te($\Delta'$), te($x$) $\simeq_{\pi}$  te($x'$), and te($y$) $\simeq_{\pi}$  te($y'$). Since, $E \in \text{Sf}(\Gamma;\Delta)$ and  $E' \in \text{Sf}(\Gamma';\Delta')$, one has that te($E$) $\simeq_{\pi}$  te($\Gamma, \Delta$) and te($E'$) $\simeq_{\pi}$  te($\Gamma', \Delta'$). Consequently, te($E$) $\simeq_{\pi}$  te($E'$).

With respect to the second property, considering the case statements in  \autoref{fig:typing_rules_cond} and \autoref{case'}, we want to prove that if $E \triangleright \text{ cond } v \hspace{2pt} \{\text{inl} (x) \Rightarrow w ; \hspace{1pt} \text{inr} (y) \Rightarrow u\}: \mathbb{C}, \Gamma'\triangleright \text{ cond } v \hspace{2pt} \{\text{inl} (x) \Rightarrow w ; \hspace{1pt} \text{inr} (y) \Rightarrow u\}: \mathbb{C}'$, and $E \simeq_{\pi} E' $, then $\mathbb{C}$ must be equal to $\mathbb{C}'$. By induction hypothesis, one has that,
\begin{enumerate}
  \item If $\Gamma \simeq_{\pi} \Gamma'$, then $\mathbb{A} \oplus \mathbb{B}=\mathbb{A}' \oplus \mathbb{B}'$;
  \item If  $\Delta, x: \mathbb{A}  \simeq_{\pi} \Delta', x': \mathbb{A}'$, then $\mathbb{C}=\mathbb{C}'$;
  \item If $\Delta, y: \mathbb{B}  \simeq_{\pi} \Delta', y': \mathbb{B}'$, then $\mathbb{C}=\mathbb{C}'$
\end{enumerate}
 
2 and 3 imply that if $\Delta \simeq_{\pi} \Delta' $, then $\mathbb{C}=\mathbb{C}'$. Since contexts $\Gamma$ and  $\Gamma'$ aren't associated with terms of types $\mathbb{C}$ and $\mathbb{C}'$, it is possible to state that if $\Delta \simeq_{\pi} \Delta $, and $\Gamma \simeq_{\pi} \Gamma'$, then $\mathbb{C}=\mathbb{C}'$. Given the requirement that the terms are well typed, it is also possible to state that  if $ \Gamma,\Delta \simeq_{\pi} \Gamma', \Delta' $, then $\mathbb{C}=\mathbb{C}'$. Moreover, given that $E \in \text{Sf}(\Gamma;\Delta)$ and  $E' \in \text{Sf}(\Gamma';\Delta')$, it follows that $\Gamma, \Delta \simeq_{\pi} \Gamma', \Delta'$. Consequently, if $E \simeq_{\pi} E' $, then $\mathbb{C}$ must be equal to $\mathbb{C}'$.

Finally, concerning the third property, it is necessary to demonstrate that the case statement in \autoref{fig:typing_rules_cond} has a unique derivation. By induction hypothesis, the terms $\Gamma \triangleright v: \mathbb{A} \oplus \mathbb{B}$; $\Delta, x: \mathbb{A} \triangleright w: \mathbb{C}$; and $\Delta, y: \mathbb{B} \triangleright u: \mathbb{C}$ each have unique derivations. Given that, according to the term formation rules in \autoref{fig:typing_rules_cond} and \autoref{fig:typing_rules_linear}, the case statement can only be derived from these previously mentioned terms, one can conclude that the case statement has a unique derivation.
...
Regarding the exchange property, for the case statement it is necessary to prove that for every judgement $\Gamma, x:\mathbb{A}, y:\mathbb{B}, \Delta \triangleright \text{ cond } v \hspace{2pt} \{\text{inl}_{\mathbb{D}}  (a) \Rightarrow w ; \hspace{1pt} \text{inr}_{\mathbb{E}}  (b) \Rightarrow u\}: \mathbb{C}$, it is possible to derive $\Gamma, y:\mathbb{B}, x:\mathbb{A}, \Delta \triangleright \text{ cond } v \hspace{2pt} \{\text{inl}_{\mathbb{D}} (a) \Rightarrow w ; \hspace{1pt} \text{inr}_{\mathbb{E}} (b) \Rightarrow u\}: \mathbb{C}$. By, induction hypothesis, and then aplying the case rule, one has that:
...
\begin{figure} [H]
  ...
  \begin{minipage}[t]{0.3\textwidth}
    \small
  $\begin{array}{c}
       E,  x:\mathbb{A}, y:\mathbb{B}, \Delta  \triangleright v: \mathbb{A} \oplus \mathbb{B} \quad \Sigma, a:\mathbb{D}, x:\mathbb{A}, y:\mathbb{B},  \Delta \triangleright w: \mathbb{C} \quad \Sigma, b:\mathbb{E},x:\mathbb{A}, y:\mathbb{B},  \Delta \triangleright u : \mathbb{C}   \quad \Gamma \in \text{Sf}(E;\Sigma)  \\
      \hline
      E,   y:\mathbb{B}, x:\mathbb{A}, \Delta  \triangleright v: \mathbb{A} \oplus \mathbb{B} \quad \Sigma, a:\mathbb{D}, y:\mathbb{B}, x:\mathbb{A}, \Delta \triangleright w: \mathbb{C} \quad \Sigma, b:\mathbb{E}, y:\mathbb{B}, x:\mathbb{A}, \Delta \triangleright u : \mathbb{C}   \quad \Gamma \in \text{Sf}(E;\Sigma)
     \\
     \hline 
     \Gamma, y:\mathbb{B}, x:\mathbb{A}, \Delta \triangleright \text{ cond } v \hspace{2pt} \{\text{inl}_{\mathbb{D}} (a) \Rightarrow w ; \hspace{1pt} \text{inr}_{\mathbb{E}} (b) \Rightarrow u\}: \mathbb{C}
  \end{array}
  $
  \end{minipage}
  \end{aligned}
  ...
  \begin{figure} [H]
    ...
    \begin{minipage}[t]{0.3\textwidth}
      \small
    $\begin{array}{c}
      \Gamma,z:\mathbb{D} \triangleright v: \mathbb{A} \oplus \mathbb{B} \quad \Delta, z:\mathbb{D}, x: \mathbb{A} \triangleright w: \mathbb{C} \quad \Delta, z:\mathbb{D}, y: \mathbb{B}  \triangleright u : \mathbb{C}   \quad E \in \text{Sf}(\Gamma;\Delta) \quad \Sigma\triangleright a: \mathbb{D}
      \\
      \hline
      \Gamma,z:\mathbb{D} \triangleright v: \mathbb{A} \oplus \mathbb{B} \quad \Delta,  x: \mathbb{A}, z:\mathbb{D}, \triangleright w: \mathbb{C} \quad \Delta,  y: \mathbb{B}, z:\mathbb{D},  \triangleright u : \mathbb{C}   \quad E \in \text{Sf}(\Gamma;\Delta) \quad \Sigma\triangleright a: \mathbb{D}
      \\
      \hline
      \Gamma, \Sigma \triangleright v[a/z]: \mathbb{A} \oplus \mathbb{B} \quad \Delta, x: \mathbb{A},  \Sigma  \triangleright w[a/z]: \mathbb{C} \quad \Delta, y: \mathbb{B}, \Sigma   \triangleright u[a/z] : \mathbb{C}   \quad E \in \text{Sf}(\Gamma;\Delta) 
      \\
      \hline
      E, \Sigma  \triangleright \text{ cond } v[a/z] \hspace{2pt} \{\text{inl} (x) \Rightarrow w[a/z] ; \hspace{1pt} \text{inr} (y) \Rightarrow u[a/z]\}: \mathbb{C}
      \\
      \hline
      E, \Sigma  \triangleright \text{ cond } v \hspace{2pt} \{\text{inl}_{\mathbb{A}}  (x) \Rightarrow w ; \hspace{1pt} \text{inr}_{\mathbb{B}}  (y) \Rightarrow u\}[a/z]: \mathbb{C}
    \end{array}
    $
    \end{minipage}
    \end{aligned}
