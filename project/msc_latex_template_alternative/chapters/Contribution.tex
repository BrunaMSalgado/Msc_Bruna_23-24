...
\begin{equation}
\begin{split}
    &q: \text{qbit}  \triangleright  I (q) =_{p} q: \text{qbit} \triangleright D_{p} (q)   \\    
\end{split}
\end{equation}
...
\begin{align}
  &\textbf{Identity}^{p} =  q: \text{qbit}  \triangleright  A_{\gamma} (q)
\end{align}
...
Now, it will be considered an imperfect implementation of an Hadamard gate, denoted as $H^{\epsilon}$. In this example,$ H^{\epsilon}$ is regarded as the composition $R_{y}(\frac{\pi}{2})\cdot P(\pi + \epsilon)$. This type of imperfection is inevitable during the implementation of quantum gates.
