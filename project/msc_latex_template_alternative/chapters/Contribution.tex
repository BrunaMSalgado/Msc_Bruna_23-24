...
\begin{equation}
  \|Q\|_{1 \text{ gen}} = \max \{ \max\{ \|Q_1 (A_1)\|_{1 \text{ gen}}  \mid  \|A_1\|_{1} = 1 \}, \hspace{2pt}\ldots \hspace{2pt}, \max \{ \|Q_n (A_n)\|_{1 \text{ gen}}  \mid  \|A_n\|_{1} = 1 \}  \},
\end{equation}
   where
...
\begin{equation}
  \begin{split}
  S \cdot Q_i & \leq  \sum_{j=1}^{m} \sum_{k=1}^{t} \underbrace{\textsc{Il} \cdot \ldots \cdot \textsc{Il}}_{t-k \times} \cdot  \underbrace{\textsc{Ir} \cdot \ldots \cdot \textsc{Ir}}_{k-1 \times} \cdot \hspace{2pt} S_{jk} \cdot Q_{ij}
  \end{split}
\end{equation}
...
\todo[inline,size=\normalsize]{ fazer referencia à definição de trace preserving para espaços com somas diretas qd eu a definir} 

If $Q_i$ is trace-preserving, then considering the definition of trace-preserving super-operators for spaces with direct sums, one has:
\begin{equation}
...
\begin{proposition} \label{prop:gen_diamond_subtraction}
  For all super-operators $Q_0, Q_1:\mathbb{C}^{o_1 \times o_1} \oplus \ldots \oplus \mathbb{C}^{o_n \times o_n}  \rightarrow \mathbb{C}^{p_1 \times p_1} \oplus \ldots \oplus  \mathbb{C}^{p_m \times p_m}$ and $P_0, P_1:\mathbb{C}^{p_1 \times p_1} \oplus \ldots \oplus  \mathbb{C}^{p_m \times p_m} \rightarrow \mathbb{C}^{q_1 \times q_1} \oplus \ldots \oplus  \mathbb{C}^{q_l \times q_l}$, it holds that:
  \begin{equation}
