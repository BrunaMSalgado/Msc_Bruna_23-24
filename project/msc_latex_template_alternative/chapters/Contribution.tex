...
\begin{definition} \label{def:gen_norm_gen_inj}
  For $Q: V \rightarrow W_1 \oplus \ldots \oplus W_m$, (where  $W_1, \ldots, W_m$  are not direct sum of vector spaces,) $\exists$ $Q_1: V \rightarrow W_1, \ldots,Q_2: V \rightarrow W_m$ such that $$Q = \underbrace{\textsc{Il} \cdot \ldots \cdot \textsc{Il}}_{m-1 \times } \cdot \hspace{2pt} Q_1 + \ldots + \underbrace{\textsc{Ir} \cdot \ldots \cdot \textsc{Ir}}_{m-1 \times } \cdot  \hspace{2pt} Q_m,$$ where $Q_1 (v) =  \underbrace{\textsc{Pl} \cdot \ldots \cdot \textsc{Pl}}_{m-1 \times } (Q (v)), \ldots, Q_M (v) =   \underbrace{\textsc{Pr} \cdot \ldots \cdot \textsc{Pr}}_{m-1 \times } (Q(v))$. As a result, considering \autoref{def:gen_norm}, the norm of the operator $Q$ correponds to:
  \begin{equation}
...
\begin{corollary}
  For all super-operators  $Q: \mathbb{C}^{o_1 \times o_1} \oplus \ldots \oplus \mathbb{C}^{o_n \times o_n}  \rightarrow \mathbb{C}^{p_1 \times p_1} \oplus \ldots \oplus  \mathbb{C}^{p_m \times p_m}$ and complex spaces $\mathbb{C}^{q_1 \times q_1} \oplus \ldots \oplus \mathbb{C}^{q_t \times q_t}$   it holds that:
\begin{equation}
  \lVert Q \rVert_{\diamondsuit \text{ gen}} = \lVert Q \otimes I_{\mathbb{C}^{q_1 \times q_1} \oplus \ldots \oplus \mathbb{C}^{q_t \times q_t}} \rVert_{\diamondsuit \text{ gen}}
\end{equation}
\end{corollary}
...
Note that if $Q$ is decomposed as  $Q=[Q_1, \ldots, Q_n]$, then for $ 1 \leq i \leq m$  $Q_i = \underbrace{\textsc{Il} \cdot \ldots \cdot \textsc{Il}}_{m-1 \times} \cdot \hspace{2pt} Q_{i1} + \ldots +  \underbrace{\textsc{Ir} \cdot \ldots \cdot \textsc{Ir}}_{m-1 \times} \cdot \hspace{2pt} Q_{im}$.
...
%\begin{equation}
 ...
 %& =  \max \{ \lVert S_{11} \cdot Q_{11} \rVert_{1} + \ldots + \lVert S_{1t} \cdot Q_{11} \rVert_{1} + \ldots +  \lVert S_{m1} \cdot Q_{1m} \rVert_{1} +  \ldots +  \lVert S_{mt} \cdot Q_{1m} \rVert_{1}, \\
  %&  \hspace{15pt} \hspace{2pt} \ldots \hspace{2pt}, \lVert S_{11} \cdot Q_{n1}\rVert_{1}  + \ldots + \lVert S_{1t} \cdot Q_{n1} \rVert_{1} + \ldots +  \lVert S_{m1} \cdot \lVert Q_{nm} \rVert_{1} +  \ldots + \lVert  S_{mt} \cdot Q_{nm} \rVert_{1} \}
  %\end{split}
%\end{equation}
...
\begin{equation}
 ...
 &\hspace{15pt} + \ldots +  \max \{ \lVert S_{11} (A_{m1}) \rVert_{1} \hspace{1pt}  \vert \lVert A_{m1} \rVert_{1} = 1 \}\}
  \end{split}
\end{equation}
It follows that $ \left\lVert S\left(\frac{P}{\lVert P \rVert_{1 \text{ gen}}}\right)  \right\rVert_{1 \text{ gen}}$ is upper bounded by $\lVert S  \rVert_{1 \text{ gen}}$. Thus, 
\begin{equation}
...
\begin{equation} \label{ineq:gen_trace_submultiplicative_O}
  \lVert S (Q (O)) \rVert_{1 \text{ gen}} \leq \lVert S  \rVert_{1 \text{ gen}} \cdot \lVert Q (O)  \rVert_{1 \text{ gen}},
\end{equation}
for every  $O \in \mathbb{C}^{o_1 \times o_1} \oplus \ldots \oplus \mathbb{C}^{o_n \times o_n}$. Considering $O = \underbrace{\textsc{Il} \cdot \ldots \cdot \textsc{Il}}_{n-1 \times} \cdot \hspace{2pt} O_{1} + \ldots +  \underbrace{\textsc{Ir} \cdot \ldots \cdot \textsc{Ir}}_{n-1 \times} \cdot \hspace{2pt} O_{m}$, it follows that  
\begin{equation}
\begin{split} 
  S (Q (O)) & = \underbrace{\textsc{Il} \cdot \ldots \cdot \textsc{Il}}_{t-1 \times} \cdot \hspace{1pt} S_{11} \cdot Q_{11} \cdot O_{1} + \ldots + \underbrace{\textsc{Ir} \cdot \ldots \cdot \textsc{Ir}}_{t-1 \times} \cdot \hspace{1pt} S_{1t} \cdot Q_{11} \cdot O_{1} + \ldots +      \\
  & \hspace{15pt}  \underbrace{\textsc{Il} \cdot \ldots \cdot \textsc{Il}}_{t-1 \times}\cdot \hspace{1pt} S_{m1} \cdot Q_{1m} \cdot O_{1} + \ldots + \underbrace{\textsc{Ir} \cdot \ldots \cdot \textsc{Ir}}_{t-1 \times} \cdot \hspace{1pt} S_{mt} \cdot Q_{1m} \cdot O_{1} + \ldots +  \\
  & \hspace{15pt} \underbrace{\textsc{Il} \cdot \ldots \cdot \textsc{Il}}_{t-1 \times} \cdot \hspace{1pt} S_{11} \cdot Q_{n1} \cdot O_{n} + \ldots+ \underbrace{\textsc{Ir} \cdot \ldots \cdot \textsc{Ir}}_{t-1 \times} \cdot \hspace{1pt} S_{1t} \cdot Q_{n1} \cdot O_{n} + \ldots +  \\
  & \hspace{15pt} \underbrace{\textsc{Il} \cdot \ldots \cdot \textsc{Il}}_{t-1 \times} \cdot \hspace{1pt} S_{m1} \cdot Q_{nm} \cdot O_{n} + \ldots + \underbrace{\textsc{Ir} \cdot \ldots \cdot \textsc{Ir}}_{t-1 \times} \cdot \hspace{1pt} S_{mt} \cdot Q_{nm} \cdot O_{n}.
\end{split}
\end{equation}
Consequentlty, \autoref{ineq:gen_trace_submultiplicative_O} can be rewritten as:
\begin{equation}
  \begin{split}
  &\lVert S_{11} \cdot Q_{11} \cdot O_{1} \rVert_{1} + \ldots + \lVert S_{1t} \cdot Q_{11} \cdot O_{1} \rVert_{1} + \ldots + \lVert S_{m1} \cdot Q_{1m} \cdot O_{1} \rVert_{1} + \ldots +  \\
  &  \lVert S_{mt} \cdot Q_{1m} \cdot O_{1} \rVert_{1} + \ldots + \lVert S_{11} \cdot Q_{n1} \cdot O_{n} \rVert_{1} + \ldots + \lVert S_{1t} \cdot Q_{n1} \cdot O_{n} \rVert_{1} + \ldots +  \\
  & \lVert S_{m1} \cdot Q_{nm} \cdot O_{n} \rVert_{1} + \ldots + \lVert S_{mt} \cdot Q_{nm} \cdot O_{n} \rVert_{1} \\
  & \leq  \lVert S  \rVert_{1 \text{ gen}} \cdot \lVert Q_{11} \cdot O_{1} \rVert_{1} + \ldots + \lVert Q_{1m} \cdot O_{1} \rVert_{1} + \ldots +  \lVert Q_{n1} \cdot O_{n} \rVert_{1} +  \ldots +  \lVert Q_{nm} \cdot O_{n} \rVert_{1}.
  \end{split}
\end{equation}

Taking the maximum over all $O \in \mathbb{C}^{o_1 \times o_1} \oplus \ldots \oplus \mathbb{C}^{o_n \times o_n}$ such that $\lVert O_i \rVert_{1} \leq 1$ yields the following inequality 



in \autoref{eq:gen_trace_submultiplicative}.
