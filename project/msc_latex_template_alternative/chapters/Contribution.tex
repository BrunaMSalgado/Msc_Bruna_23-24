...
Subsequently, it is possible to demontrate that $i^{-1}$ has a norm less or equal to 1
...
\begin{figure} [H]
  \centering
  \begin{quantikz} [column sep=0.2cm, row sep=0.5cm] 
      \lstick{$\ket{\psi}$} & \qw & \qw & \qw & \qw & \qw &\qw& \ctrl{1}& \gate{H}& \qw  & \targ{} &\qw &\qw &\qw &\qw& \qw&  \meter{} &  \setwiretype{c} &  &  &  \ctrl[vertical
wire=c]{2}  \\
...
%\begin{figure} [H]
  %\centering
  %\begin{quantikz} [column sep=0.2cm, row sep=0.5cm] 
      %\lstick{$\ket{\psi}$} & \qw & \qw & \qw & \ctrl{1}& \gate{H}& \qw  & \targ{} &\qw& \qw& \qw & \qw &\qw & \qw& \meter{} &  \setwiretype{c} &  &  &  \ctrl[vertical
%wire=c]{2}  \\
      %\lstick{$\ket{0}$} & \qw &\gate{H} & \ctrl{1}& \qw & \targ{} & \qw & \qw & \qw & \qw & \qw & \qw & \qw &\targ{} &\qw &\qw  & \meter{} & \setwiretype{c} & \ctrl[vertical
%wire=c]{1} \\
      %\lstick{$\ket{0}$} & \qw &  \qw &  \targ{} & \qw & \qw & \qw& \qw & \qw & \qw & \qw & \qw & \qw &\qw &\qw &\qw &\qw & \qw & \gate{X} & \qw & \gate{Z} \\
      %\hspace{100 pt } & \hspace{-50pt} \lstick{$\ket{0}$} 
 %\end{quantikz}
  %\caption{Quantum Teleportation Protocol with bit flip with a 50\% probability. }
  %\label{fig:dephasing}
%\end{figure}