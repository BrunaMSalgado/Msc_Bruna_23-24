...
As a result, it is necessary to define the operators considered in \autoref{fig:denotational_sem} for direct sums of vector spaces. Consider vector spaces hat are not the direct sum of other vector spaces  $V_1, \ldots, V_n$, $W_1, \ldots, W_n$, and $R_1, \ldots, R_n$ and vectors $v_1 \in V_1, \ldots, v_n \in V_n, w_1 \in W_1, \ldots, w_n \in W_n, r_1 \in R_1, \ldots, r_n \in R_n$. These operators are defined as follows:

\begin{equation}
  \begin{split}
  \ \textsc{sw}_{V_1 \oplus \ldots \oplus V_n, W_1 \oplus \ldots \oplus W_n} :  &V_{1}\otimes W_{1} \oplus \ldots \oplus  V_{n}\otimes W_{n} \xrightarrow{} W_1 \otimes V_1 \oplus \ldots \oplus W_n \otimes V_n  \\
  &(v_{1}\otimes w_{1},\ldots,v_{n}\otimes w_{n}) \mapsto (\textsc{sw}_{V_1,W_1} (v_{1}\otimes w_{1}), \ldots, \textsc{sw}_{V_n,W_n}(v_n\otimes w_{n}) ) 
\end{split}
\end{equation}

\begin{equation}
  \begin{split}
 \lambda_{ V_{1} \oplus \ldots \oplus V_{n}} :  \mathbb{C}\otimes V_{1} \oplus \ldots \oplus  \mathbb{C} \otimes V_{n} &\xrightarrow{} V_1 \oplus \ldots \oplus V_n  \\
...
\begin{equation}
  \begin{split}
   \rho_{ V_{1} \oplus \ldots \oplus V_{n}} :  V_{1} \otimes \mathbb{C} \oplus \ldots \oplus V_{n} \otimes \mathbb{C} &\xrightarrow{} V_1 \oplus \ldots \oplus V_n  \\
...
\begin{equation}
  \begin{split}
  \alpha_{V_1 \oplus \ldots \oplus V_n, W_1 \oplus \ldots \oplus W_n, R_1 \oplus \ldots \oplus R_n } :  &V_{1} \otimes (W_1 \otimes R_1) \oplus \ldots \oplus V_{n} \otimes (W_n \otimes R_n) \xrightarrow{} \\
   &(V_{1} \otimes W_1) \otimes R_1 \oplus \ldots \oplus  (V_{n} \otimes W_n) \otimes R_n  \\
  &( v_{1} \otimes (w_1 \otimes r_1),\ldots, v_{n} \otimes (w_n \otimes r_n)) \mapsto \\
  &(\alpha_{V_1, W_1, R_1} ( v_{1} \otimes (w_1 \otimes r_1)), \ldots, \alpha_{V_{n}, W_n, R_n}( v_{n} \otimes (w_{n} \otimes r_n)) ) 
\end{split}
\end{equation}



Within this model two distinct basic data types are considered: a type $\textit{bit}$ of classical bits and a type $\textit{qbit}$ of quantum bits.  The interpretation of these types is defined as  $[\![\textit{bit}]\!]=\mathbb{C}\oplus\mathbb{C}$ and $[\![\textit{qbit}]\!]=\mathbb{C}^{2\cdot 2}$. The type $\mathbb{I}$ is interpreted as $[\![\mathbb{I}]\!]=\mathbb{C}$.
...
\end{equation}
 ...
 This new model further requires that all operators be \acrshort{cptp} maps, in accordance with the definition...
...
Within this model, the super-operators take the form $Q: V_1 \oplus \ldots \oplus V_n  \rightarrow  W_1 \oplus \ldots \oplus W_m$, where each $V \in \{V_1, \ldots, V_n, W_1, \ldots, W_n\}$ is a vector space without direct sums.
...
\todo[inline, size=\normalsize]{Provar que isto é uma norma}
...
\begin{lemma} \label{lemma:max_max}
...
\begin{proof}
  %$O = \sum_{i=1}^{m} \underbrace{\textsc{Il} \cdot \ldots \cdot \textsc{Il}}_{n-i \times} \cdot   \underbrace{\textsc{Ir} \cdot \ldots \cdot \textsc{Ir}}_{i-1 \times} \cdot \hspace{2pt} O_{i} $
...
\begin{equation} \label{eq:qo}
  ...
  %\sum_{i=1}^{n} \sum_{j=1}^{m} \lVert Q_{ij} (O_{i}) \rVert_{1}.
...
%\begin{equation}
  %\begin{split}
    %S \cdot  Q = [S_1, \ldots, S_m] \cdot [Q_1, \ldots, Q_n] = [].
...
\begin{proposition}\label{lem:gen_diamond_submultiplicative}
...
\begin{lemma}\label{thm:Russo–Dye}
...
%\todo[inline,size=\normalsize]{Pôr a prova the se Q é pos Q* tb é pos. Se Q_i = (Q_i1,..., Q_nj) é pos, por uma definiçao que ainda tenho de escrever Q_i1,..., Q_nj tb são pos. Uma vez que se Q_ij é pos, Q_ij* tb é pos, Q_i* = (Q_i1*,..., Q_nj*) tb é pos.} 
...
\begin{proposition} \label{lem:gen_trace_ptp_norm1}
...
\begin{proof}
  Consider the decomposition $Q = [Q_1, \ldots, Q_n]$, where $Q_1, \ldots, Q_n$ are defined as in \autoref{def:gen_norm_gen_inj}. %Then for $ 1 \leq i \leq n$, one has that $Q_i: \mathbb{C}^{o_i \otimes o_i} \rightarrow  \mathbb{C}^{p_1 \times p_1} \oplus \ldots \oplus  \mathbb{C}^{p_m \times p_m}$ is defined as  $Q_i = \underbrace{\textsc{Il} \cdot \ldots \cdot \textsc{Il}}_{m-1 \times} \cdot \hspace{2pt} Q_{i1} + \ldots +  \underbrace{\textsc{Ir} \cdot \ldots \cdot \textsc{Ir}}_{m-1 \times} \cdot \hspace{2pt} Q_{im}$.
...
\todo[inline,size=\normalsize]{É necessário provar esta afirmação?} 
...
\begin{theorem} \label{theorem_il_ir_metric}
