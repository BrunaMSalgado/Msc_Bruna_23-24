...
\todo[inline,size=\normalsize]{Por tudo relativo a operacoes direito: interpretacao e definir operacoes nao unitarias}
...
Revisiting the example at hand, the circuit that represents the quantum teleportation protocol with a bit flip with a 50\% probability  prior to measurement measurement is depicted in \autoref{fig:teleport_bit_flip}.
...
\begin{align*}
      ...
      &\textbf{TMeasureCorrection}= q_{1}: \text{qbit}, q_{2}: \text{qbit},  q_{3}: \text{qbit} \triangleright \text{pm} \hspace{2pt} T_{X \otimes I \otimes I} (q_{1},q_{2},q_{3}) \hspace{2pt} \text{to} \hspace{2pt} x \otimes y \otimes q . \hspace{2pt}  \\
      &  \hspace{125pt} \text{cond}\hspace{2pt} \textit{meas} (x ) \\
      &\textbf{TMeasureCorrection}= q_{1}: \text{qbit}, q_{2}: \text{qbit},  q_{3}: \text{bit} \triangleright  \text{cond}\hspace{2pt} x  \hspace{2pt}  \{\text{inl} (x_{0}) \Rightarrow  (\text{cond}\hspace{2pt} y  \hspace{2pt}  \{\text{inl} (y_{0})  \Rightarrow{}  \textit{I}(q)\} ; \\
      \hspace{-28pt}
      & \hspace{335pt} \hspace{5pt} \text{inr} (y_{1}) \Rightarrow  \hspace{2pt}   \textit{X} (q)); \\
      & \hspace{240pt}\text{inr} (x_{1})  \Rightarrow  (\text{cond}\hspace{2pt} y  \hspace{2pt}  \{\text{inl} (y_{0})  \Rightarrow   \textit{Z}(q);  \\
      \hspace{-28pt}
