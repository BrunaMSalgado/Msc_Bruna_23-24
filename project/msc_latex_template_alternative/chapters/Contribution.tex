...
\right.
\]
Following the same reasoning as before in \Cref{subsec:interlude_bool} and applying the metric equation for conditionals we obtain:
\begin{equation*}
\begin{split}
  &v =  \text{case } x  
    \left\{ \begin{aligned}
...
In this category, the coproduct is defined as in $\catSet$. The distance function $d$ on the coproduct $A + B$ is given by:
  ...
  \[ \sup \{ d_A (f a, g a) \,\vert \, a \in A \} ,\] 
    ...
    %\todo[inline,size=\normalsize]{Não estou a perceber o problema de $ (a,b) \in A + B$? Nós usamos $A \otimes B$ acima tb, não podemos usar $(a,b)$?}
    \begin{align*}
