...
\begin{definition}  \label{def:gen_norm}
  ...
  \end{equation}
\end{definition}
where $\| \cdot \|_{V_i}$ is a norm associated with vector space $V_i$. When this norm is identical for all vector spaces $V_i$, the notation $\| \cdot \|_{\{V_1, \ldots, V_n\} \text{ gen}}$ is simplified to $\| \cdot \|_{V_i \text{ gen}}$.

Now it is neccesary to prove that the generalized norm is a norm.
\begin{proof}
Attending to \autoref{def:gen_norm}, demonstrating that the generalized norm is a norm is equivalent to proving that it satisfies the following properties:
\begin{enumerate}
...
Within this model, the operators take the form $Q: V_1 \oplus \ldots \oplus V_n  \rightarrow W$, where each $V_i$ is a vector space without direct sums, and $W = W_1 \oplus \ldots \oplus W_n$ is a vector space that may correspond to the direct sum of other vector spaces.  

\begin{definition}
For $Q: V_1 \oplus \ldots \oplus V_n  \rightarrow W, \exists \hspace{1pt} Q_1: V_1 \rightarrow W$, $\ldots$, $Q_n: V_n \rightarrow W$ \text{such that} $Q=[Q_1, \ldots, Q_n]$, where $Q_1= \ldots $.

The norm of the operator $Q$ is defined as follows:
\begin{equation}
\|Q\|_{\{V_1, \ldots, V_n\} \text{ gen}} = \text{max} \{\|Q_1\|_{V_1 \rightarrow W}, \ldots, \|Q_n\|_{V_1 \rightarrow W}  \}
\end{equation}
...
\begin{proof}
  Regading the positive definiteness,  given that $\|\cdot \|_{V_i \rightarrow W}$ is a norm, it follows that $\|Q_i\|_{V_i \rightarrow W} \geq 0$ and $\|Q_i\|_{V_i \rightarrow W} = 0$ if and only if $Q_i = 0$, for $1 \leq i \leq n $. Thus, $\|Q\|_{\{V_1, \ldots, V_n\} \text{ gen}} = \text{max} \{\|Q_1\|_{V_1 \rightarrow W}, \ldots, \|Q_n\|_{V_1 \rightarrow W}  \} \geq 0$ and $\|Q\|_{\{V_1, \ldots, V_n\} \text{ gen}} = 0$ if and only if $Q = [Q_1, \ldots, Q_n] = [0, \ldots, 0] = 0$.
  ...
  \begin{align*}
    \|a \hspace{1pt} Q\|_{\{V_1, \ldots, V_n\} \text{ gen}} & = \|a \hspace{1pt} [Q_1, \ldots, Q_n]\|_{\{V_1, \ldots, V_n\} \text{ gen}}\\
     &=\text{max} \{\|a \hspace{1pt} Q_1\|_{V_1 \rightarrow W}, \ldots, \|a \hspace{1pt} Q_n\|_{V_1 \rightarrow W}  \} \\
    & = \text{max} \{|a| \|Q_1\|_{V_1 \rightarrow W}, \ldots, |a| \|Q_n\|_{V_1 \rightarrow W}  \} \\
    & = |a| \hspace{1pt} \text{max} \{\|Q_1\|_{V_1 \rightarrow W}, \ldots, \|Q_n\|_{V_1 \rightarrow W}  \} \\
    & = |a| \|Q\|_{\{V_1, \ldots, V_n\} \text{ gen}}
  \end{align*}
  ...
  \begin{align*}
    \|Q + S\|_{\{V_1, \ldots, V_n\} \text{ gen}} & = \|[Q_1 + S_1, \ldots, Q_n + S_n]\|_{\{V_1, \ldots, V_n\} \text{ gen}} \\
    & = \text{max} \{\|Q_1 + S_1\|_{V_1 \rightarrow W}, \ldots, \|Q_n + S_n\|_{V_1 \rightarrow W}  \} \\
    & \leq \text{max} \{\|Q_1\|_{V_1 \rightarrow W} + \|S_1\|_{V_1 \rightarrow W}, \ldots, \|Q_n\|_{V_1 \rightarrow W} + \|R_n\|_{V_1 \rightarrow W}  \} \\
    & = \text{max} \{\|Q_1\|_{V_1 \rightarrow W}, \ldots, \|Q_n\|_{V_1 \rightarrow W}  \} + \text{max} \{\|S_1\|_{V_1 \rightarrow W}, \ldots, \|S_n\|_{V_1 \rightarrow W}  \} \\
    & = \|Q\|_{\{V_1, \ldots, V_n\} \text{ gen}} + \|S\|_{\{V_1, \ldots, V_n\} \text{ gen}}
  \end{align*}
\end {proof}

For $Q: V \rightarrow W_1 \oplus W_2$, where $V$, $W_1$ and $W_2$ do not have direct sums, $\exists$ $Q_1: V \rightarrow W_1$ and $Q_2: V \rightarrow W_2$ such that $Q = Q_1 + Q_2$, where $Q_1 (v) = \textsc{Pr}(Q(v))$ and $Q_2 (v) = \textsc{Pl}(Q(v))$.

 When $V_1, \ldots, V_n$ and $W_1, \ldots, W_n$ are complex spaces of squared matrices, the norm of the operator $Q: V_1 \otimes V_n \rightarrow W $ is defined as follows:
\begin{equation}
  \begin{cases}
    \|Q\|_{1 gen} = \text{max} \{\|Q_1\|_{1},  \|Q_2\|_{1}  \} & \text{if} \\
    \end {cases}
\end{equation}
%m
