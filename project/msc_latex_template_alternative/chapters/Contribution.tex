...
\xmapsto{\hspace{1pt} CNOT \hspace{1pt}} \quad & \frac{1}{\sqrt{2}} (|00\rangle + |11\rangle) = \ket{\Phi} \\
  \end{split}
\end{equation}
Therefore, the state of the EPR pair with an imperfect Hadamard gate is $\ket{\Phi} \bra{\Phi} = \frac{1}{2} (|00\rangle \bra{00} + \ket{00}\bra{11} + |11\rangle \bra{00} + |11\rangle \bra{11})$. Hence, the imperfect Hadamard gate does not affect the state of the EPR pair and, as a result, the distance between the ideal and noisy implementations of the EPR pair is zero, $\textbf{EPR}=_{0}\textbf{EPR}^{\epsilon}$.
