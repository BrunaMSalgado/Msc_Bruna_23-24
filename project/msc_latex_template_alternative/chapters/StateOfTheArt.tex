...
For, $\Gamma, x: \mathbb{A} \triangleright v: \mathbb{b}$ and $\Delta \triangleright w: \mathbb{A}$, the substitution $\Gamma, \Delta \triangleright v[w/x]:\mathbb{B}$ is defined below. The types of judgments are omitted as no ambiguity arises.
\begin{align*}
  ...
  \hspace{-23pt}\text{In the next two cases, }\Gamma,x: \mathbb{A}  \in \text{Sf}(&\Gamma_1,\Gamma_2), \Gamma_1 \triangleright v \text{, and } \Gamma_2, y: \mathbb{C}, z:\mathbb{D} \triangleright u  \\
...
\todo[inline,size=\normalsize]{Nas provas nós falamos das props que estão no artigo graded, as quais não incluem unique typing (a qual está no syntactic).} 
...
The properties are as follows:
 \begin{enumerate}
   ...
   \item (exchange) For every judgement $\Gamma,x:\mathbb{A}, y:\mathbb{B}, \Delta \triangleright v: \mathbb{C}$ it is possible to derive $\Gamma, y:\mathbb{B}, x:\mathbb{A}, \Delta \triangleright v: \mathbb{C}$. 
...
\todo[inline,size=\normalsize]{Fala-se de alpha-equivalence. -> when we speak of lambda terms being “equal”, we mean that they are alpha-equivalent}
\todo[inline,size=\normalsize]{Como definir commuting conversions e o que é a eq de discard?}
\todo[inline,size=\normalsize]{Usar itálico (no x) qd digo is designated x?}



The simply typed lambda calculus is a formal language that captures operations like the application of a function to an argument and the elimination of variables. To express these operations there is a set of rules known as reduction rules. These rules fall into two primary categories: the $\beta$-reductions, which perform operations and enforce the implicit meaning of the term, and $\eta$-reductions, which simplify terms by exploiting the extensionality of functions. 
There is also a secondary class of reductions - the commuting conversions. 
As a result, affine $\lambda$-calculus comes equipped with the so-called equations-in-context $\Gamma \triangleright v = w : \mathbb {A}$, depicted in \autoref{fig:equations-linear-lambda}.
\begin{figure}[H]
  \centering
  \begin{tabular}{ |ccccc| }
    \hline
$(\beta)$ & \hspace{-10pt} $\Gamma \triangleright (\lambda x : \mathbb{A}.$ $v) \hspace{1pt}  w = v[w/x]: \mathbb{B} $ & \hspace{-5pt} & ($\beta_{\otimes_{e}}$) & $ \hspace{-10pt}\Gamma \hspace{1pt}\triangleright \text{pm } v \otimes w$ to $x \otimes y.$ $u = u[v/x,w/y]:\mathbb{A}$ \\
$(\eta)$ & \hspace{-7pt} $\Gamma \triangleright \lambda x : \mathbb{A} .(v \hspace{1pt} x) = v: \mathbb{A} \multimap \mathbb{B} $  & \hspace{-5pt} & ($\eta_{\otimes_{e}}$) & \hspace{-7pt} $\Gamma \hspace{1pt}\triangleright \text{pm } v$ to $x \otimes y.$ $u[x \otimes y/z] = u[v/z] :\mathbb{A} $ \\
$(\beta_{\mathbb{I}_{e}})$ &  \hspace{-10pt} $ \Gamma \triangleright * \text { to } *.$ $v = v:\mathbb{A}$ &\hspace{-5pt} & $(\eta_{\mathbb{I}_{e}})$ & \hspace{-7pt}$\Gamma \triangleright v$ to $*$ . $w[* / z] = w[v / z]: \mathbb{A}$  \\
 $(c_{\mathbb{I}_{e}})$  &\multicolumn{4}{c|}{ $\Gamma \triangleright u[v \text{ to } \ast . w/z] = v \text{ to } \ast . u[w/z]:\mathbb{A}$ }\\
($c_{\otimes_{e}}$) & \multicolumn{4}{c|}{ $\Gamma \triangleright u[$pm $v$ to $x \otimes y.$ $w/z] =$ pm $v$ to $x \otimes y.$ $u[w/z]: \mathbb{A} $ }\\
($\text{dis}$) & \multicolumn{4}{c|}{ $\Gamma \triangleright v: \mathbb{I} = \text{dis}(x_1) \text{ to } \ast.$ $\ldots$ $\text{dis}(x_{n-1}) \text{ to } \ast \text{ dis}(x_{n}): \mathbb{ I}$ }\\
\hline
  \end{tabular}
...
It is evident that, for example, equation $(\beta)$ enforces the meaning of  $(\lambda x : \mathbb{A}.$ $v) \hspace{1pt}  w $, which is insterpreted as ``$v$ with $w$ in place of $x$". The equation $(\eta)$, on the other hand, is a simplification rule that states that a function that applies another function $v$ to an argument $x$ can be simplified to the function $v$ itself. The remaining equations follow similar reasoning. 
...
Metric equations \cite{mardare2016quantitative}, \cite{mardare2017axiomatizability} are a strong candidate for reasoning about approximate program equivalence. These equations take the form of $t=_{\epsilon} s$, where  $\epsilon$ is a non-negative rational representing the ``maximum distance" between the two terms $t$ and $s$. The metric equational system for linear lambda calculus is depicted in \autoref{fig:metric deductive system}.
\vspace{-10pt}
\begin{figure} [H]
\begin{equation*}
\begin{split}
...
The rules (refl) and (trans)  generalize the properties of reflexivity and transitivity of equality.  Rule (weak) asserts that if two terms are at a maximum distance $q$ from each other, then the distance between them is
$r \geq q$. Rule (arch) states that if $v =_r w$ for all approximations $r$ of $q$ then it is also the case that $v =_q w$. Lastly, the rule  (join) expresses that if several maximum distances between two terms are known, the actual maximum distance between them is the minimum of these distances. 

\todo[inline,size=\normalsize]{Mencionar simetria?, ver weak}
