...
The first two sections of this chapter present the mathematical and quantum computing preliminaries necessary for understanding theory of quantum information. The introduction to quantum computing is primarily based on \cite{nielsen2010quantum,watrous2018theory}, while the mathematical foundations are also based on \cite{rudin91functional}.
...
All vector spaces are assumed to be finite dimensional, unless otherwise noted.  
...
Some times the column matrix notation is used to represent vectors in $\mathbb{C}^{n}$, that is, a vector $v \in \mathbb{C}^{n}$ is written as a column matrix $$v = \begin{pmatrix} \alpha_1 \\ \vdots \\ \alpha_n \end{pmatrix}.$$
...
\begin{enumerate}
  ...
  \item $\langle v,w \rangle = \langle w,v \rangle^{\dag} $,  where \gls{dag} is the adjoint operation.
...
The inner product $ \langle v, w \rangle$ of two vectors $ v = (\alpha_1, \ldots, \alpha_n ),w = (\beta_1, \ldots, \beta_n) \in \mathbb{C}^{n}$ is defined as
\begin{equation*}
  \langle v, w \rangle = \sum_{i} \alpha_i^{\ast} \beta_i = \begin{pmatrix} \alpha_1, \ldots, \alpha_n \end{pmatrix}  \begin{pmatrix} \beta_1 \\ \vdots \\ \beta_n \end{pmatrix} ,\text{  where \gls{conj} is the complex conjugate operation.} 
\end{equation*}
...
The trace is \emph{cyclic}, that is, $\text{tr}(AB) = \text{tr}(BA)$ , and \emph{linear}, $\text{tr}(A + B) = \text{tr}(A)+\text{tr}(B), \text{tr}(s \cdot A) = s\cdot \hspace{1pt} \text{tr}(A)$, where matrices $A, B \in \mathbb{C}^{n\times n}$, and $s$ is a complex number.
...
\begin{definition}
  ...
  \begin{enumerate}
    ...
    \item Positive scalability: $\|rv\| = |r|\|v\|$ for all $v \in V$ and $r$ is a scalar;
    \item The triangle inequality: $\|v + w\| \leq \|v\| + \|w\|$ for all $v, w \in V$.
  \end{enumerate}
\end{definition}

%normed space
% Every normed space may be regarded as a metric space, in which the distance d(x, y) between x and y is II x - y 11 . The relevant properties of d are ...
  
  Every inner product space is a normed space, where the norm of a vector $v$ is defined as $\|v\| = \sqrt{\langle v, v \rangle}$.

% more broadly, a normed space is a vector space with a norm, which is a function that assigns a strictly positive length or size to each vector in the space. The norm of a vector is denoted $\|v\|$.

% produto interno disto

% cada produto interno induz uma norma |||| -> espaços normados thesis
...
Linear operators mapping an complex space $\mathbb{C}^{n}$ or $\mathbb{C}^{n\times n}$  to itself will be called \emph{square operators}, due to the fact that their matrix representations are square matrices. Therefore, those definitions given in the context of square operators are also valid for square matrices.

The following classes of operators are of particular interest in quantum information theory.

\begin{definition}
A square operator $A$ is \emph{normal} if $AA^{\dagger} = A^{\dagger}A$.
\end{definition}z

\subsection{Useful norms}


% depois de falar do produto inter de uma metriz, fazer nota sobre matrizes/operadores quadrados

%qd falar de ops unitarios falar do seu traço
...
%teorema espectral
% produto tensorial e soma direta
...
% Nota sobre notação de kets e bras sobre produto interno e externo
