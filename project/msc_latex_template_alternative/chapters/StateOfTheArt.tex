...
For a more detailed study of lambda calculus theory, the reader is referred to \eg \cite{barendregt1984lambda}. It is worth noting that this chapter includes minor contributions such as the proofs of results on conditionals that are folklore, but whose proof we could not find in the literature and the preliminary study about Booleans.
...
The expressions of the lambda calculus are called lambda terms. In the simply-typed lambda calculus, each lambda term is assigned a type. The terms without the specification of a type are called \emph{raw lambda terms}. The grammar of raw lambda terms is given by the BNF below.
...
There are certain typing rules that are not explicitly stated and whose validity follows the existing rules of the system. These are called \emph{admissible rules}.
...
Next, we will show that the programs we have defined verify certain properties of their namesake operations in Boolean algebra. Given we have just established the extensionality of the copairing, it follows that if the desired properties hold for the injections, then they also hold for any terms of type $\typeI \oplus \typeI$.
 ...
 &\inl(x) \Rightarrow x \text{ to} *. \,  \inl(z) ; \\
  ...
  Note that the idempotency property of conjunction, --- that is, $ \textbf{Conjunction} \, (\inl(*), \inl(*)) = \inl(*)$ and  $ \textbf{Conjunction} \, (\inr(*), \inr(*)) = \inr(*)$ ---   follows directly from the equalities above.
...
\begin{proof}
  ...
  \begin{align*}
    ...
    &  =  \dots  \comp (\sem{v} \otimes \id) 
          \comp (\text{exch} \otimes \text{id}) \comp \text{sp} \comp  \text{sh} & \\
...
\begin{align*}
  ...
  & \hspace{-30pt} \triangleq \left[[\![ w ]\!] ,[\![ u ]\!]\right] \comp (\text{jn}\comp \sw \oplus \text{jn} \comp \sw) \comp \dist \comp  ([\![ v [t/x] ]\!]   \otimes \text{id})  \comp \text{sp} \comp \text{sh} \\
  & \hspace{-30pt} = \left[[\![ w ]\!] ,[\![ u ]\!]\right] \comp (\text{jn} \comp \sw \oplus \text{jn} \comp \sw)  \comp \dist \comp (([\![ v]\!]  \comp \text{jn} \comp (\text{id} \otimes [\![  t ]\!] ) \comp \text{sp} )\otimes \text{id})  \comp \text{sp} \comp \text{sh} & {\text{(Induction)}} \\
  & \hspace{-30pt} =  \ldots  \comp ([\![ v]\!] \otimes \text{id}) \comp (\text{jn} \comp (\text{id} \otimes [\![  t ]\!] ) \comp \text{sp} \otimes \text{id})  \comp \text{sp}\comp \text{sh}\\
  & \hspace{-30pt} = \ldots \comp ([\![ v]\!] \otimes \text{id})  \comp (\text{jn} \comp (\text{id} \otimes [\![  t ]\!] ) \comp \text{sp} \otimes \text{id})  \comp \text{sp} \comp \text{sh} \comp \text{jn} \comp \text{sp} &  \\
  & \hspace{-30pt} = \ldots  \comp ([\![ v]\!] \otimes \text{id}) \comp (\text{jn}\comp \text{sp} \otimes   \id) \comp \text{sp}   \comp \text{sh} \comp \text{jn}\comp (\text{id} \otimes [\![ t ]\!] ) \comp \text{sp}  & {(\text{Naturality})}   \\
  & \hspace{-30pt} = \ldots  \comp ([\![ v]\!] \otimes \text{id}) \comp \text{sp}  \comp \text{sh}  \comp  \text{jn} \comp (\text{id} \otimes [\![ t ]\!] ) \comp \text{sp}  & {(\text{Coherence})}   \\
  & \hspace{-30pt} \triangleq \, [\![E,  x \vljud \text{case } v\,  \{\text{inl} (x) \Rightarrow w ;\, \text{inr} (y) \Rightarrow u\}]\!]  \comp  \text{jn} \comp (\text{id} \otimes [\![ t ]\!] ) \comp \text{sp}
\end{align*}

  With respect to the second case, \ie, $x$ lives in $\Delta$, we have: 
  \begin{align*}
  &\hspace{-30pt} [\![E, Z \vljud \text{case } v \,  \{\text{inl} (a) \Rightarrow w ;\, \text{inr} (b) \Rightarrow u\} [t/x]]\!] \\
  & \hspace{-30pt} \triangleq \left[[\![ \Delta  , Z ,  a \vljud w[t/x] ]\!] ,[\![ \Delta, Z, b \vljud u[t/x]  ]\!] \right] \comp (\text{jn}\comp \sw \oplus \text{jn} \comp \sw) \comp \dist  \comp ([\![ v ]\!]   \otimes \text{id})     \\
  ...
  & \hspace{-30pt} =   \left[[\![w ]\!] \comp  \text{jn} \comp (\text{id} \otimes [\![t]\!] ) \comp \text{sp}  ,[\![ u]\!] \comp \text{jn} \comp (\text{id} \otimes [\![t]\!] ) \comp \text{sp} \right] \comp (\text{exch} \comp \text{jn} \comp \sw  \oplus \text{exch}  & {\text{(Induction)}}  \\
  & \hspace{-20pt} \comp\text{jn} \comp \sw ) \comp \ldots \\
  & \hspace{-30pt} =  \left[[\![w ]\!] ,[\![ u]\!] \right] \comp \big(\text{exch} \comp \text{jn} \comp \sw \comp (\text{id} \otimes  \text{jn} ) \comp (\text{id} \otimes [\![t]\!]) \comp  (\text{id} \otimes \text{sp} ) \oplus \text{exch} \comp \text{jn} \comp \sw   & {(\text{Naturality,}} \\
  & \hspace{-30pt} \hspace{10pt} \comp (\text{id} \otimes  \text{jn} ) \comp (\text{id} \otimes [\![t]\!]) \comp  (\text{id} \otimes \text{sp} ) \big)\comp  \ldots & {\text{Cop. laws})} \\
  %& \hspace{-30pt} =  \ldots \comp \big(\ldots  \comp (\text{id} \otimes  (\text{jn} \comp \text{id} \otimes [\![t]\!] \comp \text{sp}) ) \oplus  \ldots  \comp (\text{id} \otimes  (\text{jn}\comp \text{id} \otimes [\![t]\!] \comp \text{sp}) )\big) \comp \dist \comp \ldots &  \\
  & \hspace{-30pt} = [[\![ \Delta  , x ,  a \vljud w]\!] , [\![ \Delta  , x ,  b \vljud u]\!]]  \comp \big(\text{jn} \comp \sw \comp (\text{id} \otimes  (\text{jn} \comp \id \otimes [\![t]\!]  \comp \text{sp}))  \oplus \text{jn} \comp  \sw      & {(\text{Exchange, }}  \\
  & \hspace{-30pt} \hspace{10pt} \comp (\text{id} \otimes  (\text{jn} \comp \text{id} \otimes [\![t]\!] \comp \text{sp}) )\big) \comp \dist \ldots  & { \text{Cop. laws})}\\
  & \hspace{-30pt} = \ldots \comp  (\text{jn} \comp \sw \oplus \text{jn} \comp \sw)  \comp \dist \comp ( \text{id} \otimes (\text{jn} \cdot (\text{id} \otimes [\![t]\!]) \cdot \text{sp} ) ) \comp ([\![v]\!] \otimes \text{id})  & {(\text{Cop. laws,}}  \\
  & \hspace{-20pt} \comp \text{sp} \comp \text{sh} & {\text{Naturality})} \\
  & \hspace{-30pt} =  \ldots \comp \dist \comp  ([\![v]\!] \otimes \text{id})  \comp ( \text{id} \otimes (\text{jn} \comp  (\text{id} \otimes [\![t]\!]) \cdot \text{sp} ) ) \comp \text{sp}\comp \text{sh} \comp \text{jn}  \comp \text{sp}    & \\
  & \hspace{-30pt} = \ldots \comp \dist \comp  ([\![v]\!] \otimes \text{id})  \comp ( \id \otimes \text{jn} \comp \text{sp}) \comp \text{sp}  \comp \text{sh}    \comp \text{jn} \comp (\text{id} \otimes [\![ t ]\!] ) \comp \text{sp} & {(\text{Naturality})}   \\
  & \hspace{-30pt} =  \ldots  \comp \dist   \comp ([\![v  ]\!] \otimes \text{id}) \comp \text{sp} \comp \text{sh} \comp \text{jn} \comp (\text{id} \otimes [\![ t ]\!] ) \comp \text{sp}  & {(\text{Coherence})}     \\ 
  & \hspace{-30pt} \triangleq  [\![E,  x\vljud \text{case } v\,  \{\text{inl} (a) \Rightarrow w ;\, \text{inr} (b) \Rightarrow u\}]\!] \comp \text{jn} \comp (\text{id} \otimes [\![ t ]\!] ) \comp \text{sp}
\end{align*}
...
\begin{proof}
  ...
  The soundeness of the equations for conditionals follow from \autoref{lem:sub_exch}, the coherence theorem for symmetric monoidal categories, naturality,  the bifunctoriality of the tensor product, and the universal property of the coproduct.   
  ...
  \begin{align*}
    ...
    & \hspace{-30pt}  \triangleq  [\llbracket w\rrbracket,\llbracket u \rrbracket] \comp (\text{jn} \comp \sw \oplus \text{jn} \comp \sw  ) \comp \dist \comp \sw \comp (\inl \comp \llbracket  v \rrbracket \otimes \text{id}) \comp \text{sp} \comp \text{sh} \\
    & \hspace{-30pt} =  \ldots \comp \dist \comp (\inl \otimes \text{id}  ) \comp ( \llbracket  v \rrbracket \otimes \text{id}) \comp \text{sp} \comp \text{sh}  \\
    & \hspace{-30pt} =  \ldots \comp \dist \comp [\inl \otimes \id, \inr \otimes \id] \comp \inl  \comp ( \llbracket  v \rrbracket \otimes \text{id}) \comp \text{sp} \comp \text{sh}  &(\text{Coproduct laws})\\
    & \hspace{-30pt} =  [\llbracket w\rrbracket,\llbracket u \rrbracket] \comp (\text{jn} \comp \sw \oplus \text{jn} \comp \sw )  \comp \inl  \comp ( \llbracket  v \rrbracket \otimes \text{id}) \comp \text{sp} \comp \text{sh} \\
    & \hspace{-30pt}  = \llbracket  w \rrbracket\comp \text{jn} \comp   \sw \comp(\llbracket v \rrbracket \otimes  \text{id}) \comp \text{sp} \comp \text{sh} & {\text{(Coproduct laws)}} \\
    &\hspace{-30pt}  =  \llbracket  w \rrbracket\comp \text{jn} \comp(  \text{id}\otimes \llbracket v \rrbracket ) \comp   \sw \comp \text{sp} \comp \text{sh} & {\text{(Naturality)}} \\
    &\hspace{-30pt}  = \llbracket w \rrbracket \comp \text{jn} \comp( \text{id} \otimes \llbracket v \rrbracket) \comp \text{sp} & {\text{(Coherence)}}\\
...
\begin{align*}
  ...
  &  \hspace{-30pt} \triangleq  [\llbracket  w [ \text{inl}(y)/x] \rrbracket, \llbracket  w [ \text{inr}(z)/x] \rrbracket] \comp (\text{jn} \comp \sw \oplus \text{jn} \comp \sw  ) \comp \dist \comp (\llbracket v\rrbracket \otimes \text{id}) \\
  &  \hspace{-20pt} \comp  \text{sp} \comp \text{sh}  \\
  &  \hspace{-30pt} = \llbracket w \rrbracket \comp [\text{jn} \comp (\text{id} \otimes\inl \comp  \llbracket y \rrbracket) \comp \text{sp} \comp \text{jn} \comp \sw , 
   \text{jn}\comp (\text{id} \otimes\inr  \comp \llbracket z \rrbracket)  \comp \text{sp} \comp \text{jn} \comp \sw  ] \comp \ldots  & {\text{(Lem. \ref{lem:sub_exch},}} \\
  &  & {\text{Cop. laws)}}  \\
  &  \hspace{-30pt} =  \llbracket w \rrbracket \comp \text{jn} \comp [ (\text{id} \otimes\inl)  \comp \sw  ,   (\text{id} \otimes\inr) \comp \sw  ] \comp \ldots  & {\text{(Cop. laws})}   \\
  &  \hspace{-30pt} = \llbracket w \rrbracket \comp \text{jn} \comp   \sw \comp  [\inl \otimes \text{id} , \inr \otimes \text{id}] \comp \dist   \comp (\llbracket v \rrbracket \otimes \text{id})  \comp  \text{sp}\comp \text{sh}  & {\text{(Naturality, }}  \\
...
\end{theorem}
 ...
 The proof of the coproduct diagram comutes follow directly from the equations $\beta_{case}^{inl}$ and $\beta_{case}^{inr}$ in \autoref{fig:equations-linear-lambda}, and substituition (\autoref{lem:exh_and_sub}). Specifically, for the left triangle in the coproduct diagram, we have that:
  ...
  $$\left[z:\typeA \oplus \typeB \vljud  \text{case } z \,\{\inl(x) \Rightarrow m[\inl(x)/ z] ; \, \inr(y) \Rightarrow m[\inr(x)/ z]\} : \typeD \right] = [z:\typeA \oplus \typeB \vljud m: \typeD].$$ 
...
is a theorem of $T$ if and only if it is satisfied by all first-order models of the theory.
 ...
 \begin{proof}
  ...
  \begin{align*}
    \big[z:(\typeA \otimes \mathbb{\typeB}) \otimes \mathbb{\typeD} \vljud \text{pm } z \text{ to } x \otimes d. \text{ case } x\,\{ &\inl_{\typeB}(a) \Rightarrow \inl_{\typeB \otimes \typeD}(a \otimes d); \\
