...
\begin{proof}
Since these properties are established in \cite[Theorem 2.3]{dahlqvist2023complete} for the lambda calculus without conditionals, we only need to focus on them.
...
\begin{proof}
  Once again, these properties are established in \cite[Theorem 2.1]{dahlqvist2022syntactic} for the lambda calculus without conditionals, so we only need to focus on them.
...
\end{align*}
  ...
  & = w'
    ...
    For the second equality we note that in order for the equation no make sense the context of $w'$ must be empty.
    \begin{align*}
      & \textbf{Conjunction} \, [\inr(*)/v,w'/w ] \\
      & = \text{case } \inr(*)\,
  \{\inl_{\typeI}(x) \Rightarrow x \text{ to} *. \, w' ;
  \, \inr_{\typeI}(y) \Rightarrow y \text{ to} *. \,  \text{dis}(w') \text{ to} *. \inr_{\typeI}(*)
  \} \\
  & =  * \, y \text{ to} *. \,  \text{dis}(w') \text{ to} *. \inr_{\typeI}(*) & (\beta_{case}^{inr}) \\
  & = \text{dis}(w') \text{ to} *. \inr_{\typeI}(*) & (\beta_{\mathbb{I}_{e}}) \\
  & = * \text{ to} *. \inr_{\typeI}(*) &  (\eta_\text{dis}) \\
  & =  \inr_{\typeI}(*)
    \end{align*}
...
\begin{proof}
  ...
  \begin{equation*}
  \begin{split}
  & \textbf{Disjunction} [\textbf{Negation} \, [v'/v] / v, \textbf{Negation} \, [w'/v] / w ] \\
  & \triangleq \text{case }  \, 
    \text{case } v  
    \left\{ \begin{aligned}
