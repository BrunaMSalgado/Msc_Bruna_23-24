...
\emph{multiplicative} fragment of linear logic. Here we start
investigating the incorporation of \emph{additive} structure to this body of
...
the continuity properties assumed from the quantale
in~\cite{dahlqvist22,dahlqvist2023syntactic} are key for achieving
completeness. We show furthermore that even if the well-known Archimedean rule
(often problematic) is dropped one still retains `approximate completeness'. We
briefly illustrate our extended framework at work for 
...
thus emphasising the emergent shift from ``seeing program semantics as the
...
12]{cats}, in other words it has all infima. We will thus assume that the basis
$B$ is closed under binary infima.  Several examples of quantales that satisfy
these constraints are presented and discussed
...
programming.  Next, as previously mentioned the assumption that $\mathcal{V}$
is continuous will play a key rôle in (approximate) completeness. The key point
is that the continuity of $\mathcal{V}$ entails that for every $x \in
\mathcal{V}$ the operation,
\[
        x \wedge (-) : \mathcal{V} \to \mathcal{V}
\]
is continuous, \ie\ it preserves directed
suprema~\cite[Proposition I-1.8]{gierz03}.
