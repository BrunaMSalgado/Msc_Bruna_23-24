...
\title{On the cartesian structure of quantalic $\lambda$-calculus} %TODO Please add
...
%Program equivalence and its underlying theories traditionally rely on a binary
%notion of equivalence: two programs are either equivalent or they are not. While this
%dichotomy is often sufficient for classical programming, it proves too
%coarse-grained for other computational paradigms. We extend previous work
%on a quantalic equational system for
%linear $\lambda$-calculus to the Cartesian domain.
%we extend the framework by incorporating a metric equation for the conditional
%statement. We prove that this extension is sound and complete. This result
%allows us to explore models within quantum computing and probabilistic
%computation suitable for reasoning about approximate program equivalence in
%this setting.
